...
> Layer: Architecture
> Governing authority: **D-008** (desktop is a Native AOT deliverable), **V-05a** (Avalonia AOT evidence)
...
| # | Rule |
|---|---|
| AO-01 | **`PublishAot` on the executable; `IsAotCompatible` on every library it consumes** (**V-05a**). |
...
| AO-07 | **Every third-party control is verified by a real AOT publish** before acceptance — the unbounded exposure identified by **V-05a**. *Owner: owning platform work-package owner. Trigger: before accepting any third-party control into an AOT deliverable.* |
...
| AO-12 | **Generic instantiations are pre-generated**, so unbounded generic fan-out is avoided for artifact size. |
...
| MV-06 | **A remote modification produces the same projected update as a local one** (`LY-06` in the overview). |
| MV-07 | **View models are not shared with mobile** (**D-021**). |
...
| TH-08 | **A synchronous UI-thread block beyond the responsiveness threshold is a defect** (`RS-03` in the quality contract). |
...
| WI-02 | **One document has exactly one write authority at a time** (`WN-04` in the shared desktop requirements). Several windows on one document share one session. |
...
| WI-05 | **`InstanceId` is unique per launch; `AppId` is stable** (`PM-07` in the overview). |
| WI-06 | **Multi-window is the default capability; multi-process is an explicit extension** (`SI-21`, `SI-22`). |
| WI-07 | **Window and layout physical state is device-local** (`SI-24`). |
...
| # | Rule |
|---|---|
| PR-01 | **Local persistence uses an AOT-safe data access path** with explicit or generated mapping — not a reflection-driven ORM runtime as an irreplaceable dependency (**D-008**, `I3 §12.1`). |
...
| PR-07 | **Writable database files are never shared between products** (`P-09`). |
...
**Crash-loop protection** enters a read-first Safe Start mode after repeated start-up failure, which never modifies canonical content and never deletes user data (`CR-06` in the quality contract).
...
| LT-02 | **The task owner is the product that actually performs the work** (`OW-01` in the AI requirements). |
| LT-03 | **Task state is persisted** and, after a restart, is recovered, failed, or explicitly marked interrupted. |
| LT-04 | **Progress is a monotonic best estimate** (`PR-01` there). |
...
| LT-06 | **Cancellation is a request** (`CN-01` there). |
...
| # | Rule |
|---|---|
| CC-01 | **Each product owns its own cloud client** for its own data (**D-010**). ArcChat is not a proxy. |
| CC-02 | **The typed HTTP client uses the generated-only registration and entry point.** The reflection package is absent; its diagnostic is build-breaking (**F-026**). |
...
| CC-07 | **A local sync outbox is durable** (`SY-32` in the cloud requirements) and survives a crash immediately after save. |
...
| # | Rule |
|---|---|
| SH-01 | **The shell holds no product domain state** (`SI-28`). |
| SH-02 | **A product composes the shell; the shell never dictates the product's workspace layout** (`SI-01`). |
| SH-03 | **Every command, menu item, toolbar button, shortcut and palette entry resolves to one command identity** (`CM-02`). |
| SH-04 | **Undo state belongs to the product** (`CM-06`); the shell provides the command plumbing only. |
...
| # | Rule |
|---|---|
| SU-01 | **Nothing in the background column may block reaching a usable workspace** (`SU-01`–`SU-03` in the quality contract). |
...
| SU-04 | **A first-launch account gate is prohibited** (`ID-01`). |
...
**Closing a window is not quitting, and quitting is not stopping background work** (`LF-01`). Where genuine work continues — a capture, a render — the product says so and offers a choice (`LF-04`).
...
| PF-05 | **Image, frame and analysis caches have a budget, an eviction policy and pressure feedback** (`MM-03` in the quality contract). |
| PF-06 | **Native memory enters the budget and the telemetry**; the managed heap alone is insufficient (`MM-01` there). |
...
| **D-008** | Desktop as a Native AOT deliverable with trim/AOT-safe dependencies |
| **V-05a** | Avalonia AOT requirements and the third-party control publish gate |
| **F-026** | The typed HTTP client entry point and reflection-package prohibition |

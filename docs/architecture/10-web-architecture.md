...
> Layer: Architecture
> Governing authority: **D-007** (technology and rendering boundary), **D-014** (surface inventory), **D-015** (canonical account origin)
...
| # | Rule |
|---|---|
| BO-01 | **Public marketing, legal, download and information pages render as static HTML and CSS without waiting for the .NET runtime or WebAssembly** (**D-007**). |
...
| BO-04 | **`RunAOTCompilation=false`** unless a measured benchmark and an explicit decision prove otherwise (**D-007**). |
...
| # | Rule |
|---|---|
| SG-01 | **Product catalogue and release metadata come from one source of truth** (`ST-01`, `ST-02` in the web requirements). The generator consumes it; it never re-states versions. |
| SG-02 | **Pricing comes from one source of truth** (`ST-03` there). |
| SG-03 | **Above-the-fold content is present in the delivered HTML.** No client script is required to render it. |
| SG-04 | **Locale-scoped URLs with correct alternate-language annotations**; no client-only language switching and no trapping automatic redirect (`IL-01` there). |
| SG-05 | **No blocked third-party resource on the critical path** — fonts, script hosts, analytics and verification providers all chosen for global reachability (`IL-02` there). |
...
| # | Rule |
|---|---|
| WA-01 | **One codebase, several deployments** (**D-014**). A deployment profile selects the feature set, navigation and branding. |
| WA-02 | **Deployments do not share state, storage or cookies** (`WB-02` in the security architecture). |
| WA-03 | **Serialization uses source-generated contexts** for every DTO, matching the desktop and mobile clients. |
| WA-04 | **The typed HTTP client uses the generated-only entry point**; the reflection package is absent (**F-026**). |
...
| WA-07 | **On a realtime message the client re-reads authoritative state where it needs full fidelity** (`RL-04` in the cloud architecture). |
...
| `account.arcforges.com` | Web App (Account profile) | Required | **Canonical account origin** (**D-015**) |
...
| # | Rule |
|---|---|
| SM-01 | **`arcforges.com/account` is a permanent redirect and never a second account application** (**D-015**). |
| SM-02 | **Each origin has its own CSP, cookie policy, CORS policy and CSRF posture.** |
| SM-03 | **No broad parent-domain authentication cookie exists** (**D-015**). |
| SM-04 | **The status page is hosted independently of ArcForges Cloud**, with an emergency alternate URL (`SL-07` in the cloud product requirements). |
...
| AU-02 | **A long-lived access token is never held in storage readable by arbitrary scripts** (`WB-05` in the security architecture). |
| AU-03 | **The concrete token-handling deployment — cookie-based backend-for-frontend versus in-memory tokens — is settled by a decision record**, and both must satisfy `AU-02`. |
| AU-04 | **A web session is shorter-lived and less trusted than a desktop session** (`OF-07` in the companion requirements). |
| AU-05 | **A new browser does not immediately hold high-risk approval capability** (`OF-08` there). |
...
| CS-02 | **No secret is compiled into the bundle** (`WB-04` there). Any value in the bundle is public. |
| CS-03 | **Cross-origin policy is an explicit allowlist** (`WB-06` there). |
| CS-04 | **Uploads are content-type-, size- and format-validated with a quarantine area, and never executed server-side** (`WB-07` there). |
| CS-05 | **Realtime transport logs redact tokens** (`WB-08` there). |
| CS-06 | **Preview rendering of user content is sandboxed** — untrusted HTML or documents never execute in the application origin (`EX-06` in the cloud requirements). |
| CS-07 | **There are no public share links in V1** (`SE-08` in the web requirements). Links are authenticated and private. |
| CS-08 | **A cloud resource URL verifies permission at access**, and a denial does not disclose existence where that would leak (`LN-04` in the companion requirements). |
...
| # | Rule |
|---|---|
| OR-01 | **Web offline is minimal and honest** (`OF-05` in the companion requirements): the application states it is offline and preserves unsent input; it does not pretend to work. |
| OR-02 | **A cloud outage does not blank the application.** It reports which capabilities are unavailable, with reasons. |
| OR-03 | **Realtime loss degrades to polling authoritative state**, then backfills on reconnection (`RL-04` in the cloud architecture). |
...
| AL-02 | **Keyboard-only completion of every core workflow.** |
| AL-03 | **Every user-visible string is localisable**, including in generated static pages. |
| AL-04 | **Locale-safe data handling**: canonical storage, localised presentation (`LO-06`–`LO-09` there). |
| AL-05 | **Public site performance is a product requirement** measured at the 75th percentile, and the above-the-fold render does not depend on the client runtime (`IL-03` in the web requirements). |
...
| # | Rule |
|---|---|
| AN-01 | **Minimal, privacy-preserving analytics** (`PV-05` in the security requirements). |
...
| AN-04 | **Application telemetry is consented and carries no user content** (`OB-05` in the cloud architecture). |
...
| # | Rule |
|---|---|
| BD-01 | **Build once, promote the same artifact** (`EN-10` in the cloud product requirements). |
...
| BD-05 | **The WebAssembly publish and its AOT-build verification are part of the release matrix** (`PM-04` in the quality contract). |
...
| **D-007** | Static public rendering; one WebAssembly application; prohibited technologies |
| **D-014** | The surface inventory and "a hostname is not an application" |
| **D-015** | Canonical account origin and per-origin boundary policy |

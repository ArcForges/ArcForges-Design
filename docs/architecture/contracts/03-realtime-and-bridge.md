...
> Governing authority: [`00-operation-catalogue.md`](00-operation-catalogue.md), **D-010**, [`../05-cloud-architecture.md`](../05-cloud-architecture.md) `§7`, `§10`
...
Every event carries `{ subscriptionKey, seq, workspaceId, occurredAt, correlationId }` plus its own payload. `seq` is per subscription (`RV-05`), which is what makes a gap detectable.
...
| # | Rule |
|---|---|
| RE-01 | **No event carries authoritative state** (`SO-02`). Every payload above is an identifier plus enough metadata to decide whether to act. |
| RE-02 | **No event carries an object body, a message body, or document content.** |
| RE-03 | **`task.progress` is explicitly lossy.** Losing every progress event must not affect the recorded outcome (`WP-16.06`). |
...
| # | Rule |
|---|---|
| SB-01 | **Permission is checked at subscribe and re-checked when permission changes.** Losing permission stops delivery immediately and tells the client (`WP-24.01`). |
...
| # | Rule |
|---|---|
| GP-01 | **A gap is never ignored.** It is recorded as telemetry and triggers reconciliation (`WP-24.02`). |
| GP-02 | **Reconciliation is scoped, not a blanket resync.** A `sync.changed` gap pulls the change feed from the last cursor; a `task` gap re-reads that task. |
| GP-03 | **Reconnection reconciles unconditionally**, because the client cannot know what it missed while disconnected. |
| GP-04 | **Convergence is verifiable** — after reconciliation, client and server state compare equal for the subscription's scope (`WP-24.03`). |
...
| Condition | Behaviour |
|---|---|
| Realtime unavailable | Poll authoritative state at a bounded interval; **degradation is visible** (`WP-24.05`) |
| Realtime flapping | Exponential backoff with jitter; no reconnect storm (`WP-24.00`) |
| Cloud fully unavailable | Client reports which capabilities are unavailable with reasons; **never blanks** (`OR-02`) |
...
**D-010** forbids Cloud from connecting to a device. The bridge is therefore pull-and-answer, and its shape is a direct consequence of that decision rather than a preference.
...
| # | Rule |
|---|---|
| BR-01 | **The desktop re-evaluates every request against local policy, the local capability registry, local permission grants and the local actor chain** (`WP-26.02`). |
...
| BR-04 | **An operation requiring local presence cannot be satisfied through the bridge** (`AZ-01`), because the requester is by definition not present. |
...
| # | Rule |
|---|---|
| BI-01 | **`bridge.submitResult` is idempotent on `(taskId, attemptId)`.** Re-submission after a lost response has one effect (`WP-26.03`). |
| BI-02 | **A request delivered but unanswered before a desktop crash is re-delivered** on the next pull, and the local command log makes re-execution a no-op if it already ran (`§1.2` of the desktop data model). |
| BI-03 | **This is the one place where local and cloud idempotency must agree**: the desktop's `command_log` and Cloud's `attempt` row both key on the same `CommandId`. A mismatch here is the defect class most likely to cause a duplicate real-world effect, and `WP-26.03` tests it specifically. |
| BI-04 | **An expired request closes with a typed reason**, never ambiguously (`WP-26.05`). |
| BI-05 | **An offline target queues visibly** with its expiry shown to the requester (`WP-26.05`). |
...
| # | Rule |
|---|---|
| BE-01 | **An `unknown` effect on a non-idempotent capability surfaces a decision** rather than retrying (`WP-16.02`). |
...
| Not | Why |
|---|---|
| A tunnel | Cloud never opens a connection to a device (**D-010**) |
...
| An authorization channel | Local re-authorization is mandatory and final (`BR-01`) |
...
| # | Obligation | Where |
|---|---|---|
| RV-01 | Every induced gap is detected, recorded, and reconciled to verified convergence | `WP-24.02`, `WP-24.03` |
| RV-02 | Duplicate and out-of-order delivery cause no corruption | `WP-24.02` |
| RV-03 | Permission loss stops delivery immediately | `WP-24.01` |
| RV-04 | No event payload carries authoritative state or a body | Contract policy test |
| RV-05 | **No cloud-initiated connection to a device exists anywhere**, verified structurally and by runtime network observation | `WP-26.01`, `WP-31.06` |
| RV-06 | A cloud-approved request is still refused when local policy denies it | `WP-26.02` |
| RV-07 | One request produces one effect under duplicate delivery, lost result and mid-execution crash | `WP-26.03` |
| RV-08 | A local-presence operation cannot be completed through the bridge | `WP-26.04` |
| RV-09 | An offline target queues visibly and expires with a typed reason | `WP-26.05` |

...
Media uses ArcMediaNative; Colour ArcSlateColorNative; Image ArcSlateImageNative; Otio ArcSlateOtioNative. New Instruments/Pdf/Graphics libraries use ArcInstrumentsNative/ArcPdfNative/ArcGraphicsNative. Existing APPLE-only ArcGraphicsMetalNative and arc_metal_get_abi_version/get_build_info/get_last_error retain their identity and ABI1.0. They are probe compatibility, not functional acceleration APIs. ArcGraphicsNative supplies the required cross-RID CPU surface; any Metal implementation is its private backend and cannot be claimed implemented by calling those three probes. Managed packages are `ArcForges.Native.<Capability>` plus exactly one explicitly referenced `Runtime.<rid>` sibling; Native.Abstractions owns common status/handles only. Existing Media libusb assets stay compatible; future Instruments closure records and deduplicates any identical shared-library filename by hash, and conflicting same-name dependencies fail packaging. Consumer never invokes CMake/vcpkg.
...
`sandbox.buffer.v1` is the typed gRPC buffer-seal/ack profile of ContentSandboxService in [local09](09-local-grpc-and-sandbox.md#4-restricted-launch-and-os-resources). Parent provisions immutable input and at most3slots×64MiB before parsing through finite OS handle handoff. All grant/read/seal/ack/cancel/status controls use authored proto/native gRPC, including macOS. No raw pointer is serialized. Parent binds generation/sequence, validates geometry/stride/coverage, copies the declared bytes into admitted private memory and checks digest on that copy before use. Slot reuse follows matching acknowledgement; cancellation, timeout or parent death invalidates the invocation and cleans every mapping. Pixel tiles<=2048squareRGBA32f; full-frame memory is separately admitted. The complete parser operation bindings, lifetime and bounded OTIO resource chunks are in registry04/local09; a private XPC command protocol is not permitted.

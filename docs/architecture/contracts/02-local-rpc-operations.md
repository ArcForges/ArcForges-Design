...
| NO-04 | **A write takes and returns the composite local token `(acked_rev, local_rev)`**, not a bare revision (`RV-C3`, `RV-C4` of the desktop data model), and enqueues a `sync_outbox` row. It does **not** return a Cloud acknowledgement (`PE-04`): the caller learns the edit is durable on this device, which is a different fact from acknowledged by Cloud. Passing only `acked_rev` would let two local callers overwrite each other between acknowledgements; passing `local_rev` to Cloud would conflict on every second edit. |

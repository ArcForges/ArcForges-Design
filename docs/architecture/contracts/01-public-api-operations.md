...
Step-up, credential management, recovery, revoking other sessions and workspace selection use the existing public Identity/Workspace operations through the cookie-auth adapter with the same authorization and idempotency rules. gRPC-Web stream Origin/CSRF validation are explicit transport requirements, not new business operations. No endpoint accepts a caller-supplied forwarding URL.

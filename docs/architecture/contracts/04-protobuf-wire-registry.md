...
ExecutionLocus = cloud, device, either; EffectKind = pureRead, localWrite, cloudWrite, externalWrite; BindingProtocol = publicGrpc, localGrpc, extensionGrpc, providerHttp, machineTool.
...
| `ToolRequest` | `1 toolRequestId:Id`; `2 taskId:Id?`; `3 runId:Id`; `4 stepId:Id`; `5 attemptId:Id`; `6 commandId:Id`; `7 targetDeviceId:Id?`; `8 capability:Key`; `9 arguments:CapabilityArguments`; `10 context:ContextRef[]`; `11 actor:ActorChain`; `12 approvalId:Id?`; `13 expiresAt:Instant`; `14 state:Key`; `15 owner:ExecutionOwner` | queued/delivered/answered/expired/refused; owner required, legacy taskId present iff matching Task owner. Pure-read ChatTurn tools have no fabricated Task; targetDevice only for device tool. Sender never chooses effective actor. |
...
| `ConversationView` | `1 conversationId:Id`; `2 branchId:Id`; `3 title:Name`; `4 revision:Revision`; `5 lastMessageAt:Instant?`; `6 projectId:Id?`; `7 pinned:bool`; `8 archived:bool`; `9 temporary:bool` | Temporary conversation is excluded from ordinary list/history. Branch ancestry immutable; metadata writes expectedRev. |

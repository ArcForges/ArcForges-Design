...
| `AuthChallenge` | `1 flowId:Id`; `2 method:AuthMethod?`; `3 challenge:bytes`; `4 rpId:Key?`; `5 expiresAt:Instant`; `6 options:WebAuthnOptions?`; `7 providerId:Key?`; `8 redirectUrl:string?`; `9 purpose:AuthPurpose`; `10 recoveryMethod:RecoveryMethod?` | Purpose recover requires recoveryMethod and omits method; other purposes require method and omit recoveryMethod. Challenge <=32bytes; no secret in email/password challenge. OIDC redirect is generated from configured issuer plus server flow, never caller URL. |
...
| `DeviceView` | `1 deviceId:Id`; `2 name:Name`; `3 platform:Key`; `4 trust:TrustLevel`; `5 remoteEnabled:bool`; `6 revokedAt:Instant?`; `7 presence:Key`; `8 revision:Revision`; `9 lastSeenAt:Instant?`; `10 installed:InstalledProduct[]`; `11 presenceExpiresAt:Instant?` | Durable trust/grants and expiring presence are distinct. Installed list<=16products; offline is not uninstalled. Instances/capabilities use paged device.getCapabilities. |
| `InstancePresence` | `1 instanceId:Id`; `2 product:Key`; `3 capabilityVersion:Key`; `4 eligible:bool`; `5 appVersion:Key`; `6 readiness:Readiness`; `7 reason:ReasonCode?`; `8 generation:uint64`; `9 startedAt:Instant`; `10 capabilityDigest:Hash` | Current instance observation only. Eligibility is server-computed, never elevated by a heartbeat. Capability details fetched from registered version/digest. |
...
| `TurnOptions` | `1 taskId:Id?`; `2 profileId:Id`; `3 maxBudget:uint64?`; `4 mode:ChatMode`; `5 turnId:Id?`; `6 modelId:ModelId?`; `7 targetDeviceId:Id?`; `8 allowWebSearch:bool` | Agent requires taskId and omits turnId; ordinary/temporary require turnId and omit taskId. Budget cannot enlarge policy. Model must be admitted by the profile. Search defaultfalse, explicit selection persists in the conversation preference only by user action. |
...
| `TableBlock` | `1 rows:TableRow[]` | <=200rows,50columns,10000cells; at most200changed cells per semantic mutation. Whole imported snapshots use the staged owner import path. No formulas or implicit row/column IDs. |
...
| `ScopeConfiguration` | `1 configurationId:Id`; `2 channels:ChannelDefinition[]`; `3 parserProfile:Key`; `4 revision:NativeContentRev`; `5 framing:FrameConfiguration`; `6 trigger:TriggerConfiguration?` | Closed framing/trigger profiles from architecture26. Configuration change starts a new immutable source segment. |
...
| `InlineAtom` | `1 offset:uint32`; `2 mention:LinkSpec`; `3 math:MathContent`; `4 footnote:LinkSpec`; `5 origin:ContentOrigin?`; `6 inlineId:Id` | oneof content={mention,math,footnote}; unique offset pointing at U+FFFC. Stable document/block targets, no forged user membership. |
...
| `SlateSelection` | `1 projectId:Id`; `2 sequenceId:Id`; `3 range:MediaRange`; `4 items:Id[]`; `5 revision:NativeContentRev` | Pinned ticks/range; <=10000items or immutable typed selection resource. |
| `EphemeralSelection` | `1 instanceId:Id`; `2 generation:uint64`; `3 selectionId:Id`; `4 expiresAt:Instant` | <=60seconds from snapshot; instance death/generation change invalidates, no regeneration from stale UI coordinates. |
...
| `DeviceCapabilityView` | `1 instance:InstancePresence`; `2 descriptor:CapabilityDescriptor`; `3 remoteAllowed:bool`; `4 reasons:ReasonCode[]`; `5 observedAt:Instant`; `6 expiresAt:Instant` | Intersection of current trust, grants, instance readiness and user policy. |
...
| `TurnPromotionPreview` | `1 turnId:Id`; `2 previewHash:Hash`; `3 taskInput:TurnInput`; `4 profileId:Id`; `5 proposals:ToolProposal[]`; `6 expiresAt:Instant` | <=15minutes, no write execution before explicit promotion and ordinary Task approval. |
...
| `WebSearchRequest` | `1 query:Text`; `2 count:uint32`; `3 language:Key`; `4 country:CountryCode`; `5 consent:SourceConsentRef?` | First-party search profile: query1–512scalars,count1–10(default5),strict safe search,offset0. Consent binds explicit user search selection; no hidden local data egress. |
| `WebSearchHit` | `1 rank:uint32`; `2 title:Name`; `3 url:string`; `4 snippet:Text`; `5 fetchedAt:Instant`; `6 provider:Key` | http(s) absolute URL; snippet<=2048scalars; untrusted evidence, never an instruction. |
| `WebSearchResult` | `1 requestHash:Hash`; `2 hits:WebSearchHit[]`; `3 providerRequestId:Key?`; `4 completedAt:Instant` | <=10 ordered hits and256KiB; provider=braveWebV1, no synthetic success on unavailable. |
...
| `SerialSource` | `1 baud:uint32`; `2 dataBits:uint32`; `3 parity:Key`; `4 stopBits:uint32`; `5 flowControl:Key` | Positive supported baud; bits5-8, parity none/odd/even, stop1/2, flow none/rtsCts/xonXoff; unsupported OS configuration refuses. |
| `SourceEndpoint` | `1 host:string`; `2 port:uint32`; `3 bindAddress:string?`; `4 tls:bool` | Local configured network acquisition, explicit endpoint grant; port1-65535, no URL redirects. TLS requires trusted certificate/host validation. |
...
| `FrameConfiguration` | `1 kind:Key`; `2 start:Bytes`; `3 end:Bytes`; `4 escapeByte:uint32?`; `5 delimiter:uint32?`; `6 header:bool`; `7 frameBytes:uint32?`; `8 byteOrder:Key`; `9 fields:FrameField[]`; `10 checksum:ChecksumSpec?` | delimitedText/csvLine/jsonLine/fixedBinary/canonicalReplay. Max frame1MiB; fields<=64; kind-specific inactive fields refuse; architecture26 fixes parsing/recovery. |
...
| `ChecksumSpec` | `1 algorithm:Key`; `2 input:ByteRange`; `3 offset:uint32`; `4 byteOrder:Key` | crc16CcittFalse/crc32IsoHdlc; checksum bytes excluded from input; fixed vectors in architecture26. |
...
| `SpectrumOptions` | `1 size:uint32`; `2 resampleRate:Rational?`; `3 profile:Key` | spectrum.v1 periodic Hann, mean removal and amplitude/PSD normalization; exact bounds in architecture26. |
...
| `ConnectorChallenge` | `1 flowId:Id`; `2 connectionId:Id`; `3 authorizationUrl:string?`; `4 expiresAt:Instant` | HTTPS allowlisted provider redirect, PKCE/state callback,10minute single-use flow; personalToken flow has no URL. |
...
| task.readStream | ExecutionService.ReadOutput (typed Task-owner alias) | Binary gRPC-Web and owner/cursor recovery in annex10; C# Task remains business authority |
...
C# uses Grpc.Tools 2.83.0, Google.Protobuf 3.36.1, Grpc.AspNetCore/Grpc.Net.Client/Grpc.AspNetCore.Web2.83.0. Generated registration is explicit, no runtime service discovery or reflection serialization. Browser uses @bufbuild/protobuf and protoc-gen-es2.14.1 plus @connectrpc/connect/connect-web2.2.0 createGrpcWebTransport. The committed Grpc.Tools package supplies protoc for both generators, ensuring one compiler rather than independently downloaded protoc. Contract generation runs locked restore → protoc C#/grpc plus protoc-gen-es TS and Java/Kotlin-lite/coroutine stubs → descriptor validation → compatibility check against released descriptor → generated validator/tool-schema projection → C#/TS/Kotlin fixtures → pack. Generated descriptor sets and package integrity hashes are published; a second generation must produce no diff.

All public clients use binary gRPC-Web with the selected generated C#/TS/Connect Kotlin clients in [annex10](10-application-scope-and-streams.md). Validate five-byte framing, bounded message counts/lengths, terminal status trailers and cancellation. Unary request/response max4MiB; list/event responses max256KiB; streaming frames max32KiB. HTTP success is not RPC success. Java/Kotlin-lite messages and Connect Kotlin clients replace the native-grpc-only Android binding; no TS/Hermes Mobile runtime.

The only public business route is /api/arcforges.<domain>.v1.<Service>/<Method>, binary gRPC-Web for every platform. Ordinary products expose no local gRPC server. Private parent-owned helper/extension controls use [annex09](09-local-grpc-and-sandbox.md), OS-authenticated Named Pipe/UDS and explicit launch-scoped registration.

Default public unary deadline10s (owner-approved synchronous measurement max30s); private helper bounds remain16active/64queued and4MiB. ResourceRef/verified object transfer carries large results. There is no product registration prerequisite for launch, own-app assistant use or direct Cloud synchronization; application scope/target and stream recovery follow annex10.
...
EventService.Poll and EventService.Watch establish the existing scoped hint feeds under annex10; neither grants new authority. With absent cursor it returns an empty page and signed current cursor with resetRequired=true; client performs its authoritative snapshot before continuing. D1 holds bounded non-authoritative hint envelopes per subscription (24-hour TTL, latest10,000 events). Publishing advances a transaction-guarded per-subscription sequence with the inserted hint; missing/expired intervals or a changed auth epoch produce resetRequired, never a successful incomplete page. A positive empty poll advances only to the observed committed head. Every call rechecks scope/owner/device, and byte-limit paging returns the last delivered cursor. Loss of the entire hint store still converges through the established authoritative reads.

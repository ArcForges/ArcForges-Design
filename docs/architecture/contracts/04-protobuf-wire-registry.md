...
| `PageState` | `1 nextCursor:Cursor?`; `2 hasMore:bool` | Cursor binds realm/workspace/actor/filter/sort/schema/snapshot; 15-minute validity unless profile specifies shorter. |
| `RequestMeta` | `1 commandId:Id?`; `2 expectedRev:Revision?`; `3 correlationId:Id`; `4 workspaceId:Id?`; `5 expectedLocal:LocalNotesVersion?`; `6 expectedNative:NativeContentRev?` | Exactly the declared revision kind; commands require stable commandId; authentication NI endpoints use flow identity. |
| `ResponseMeta` | `1 resultRev:Revision?`; `2 entitlementVersion:int64?`; `3 warnings:ReasonCode[]`; `4 correlationId:Id` | No secret or authority assertion. |
...
| `AuthChallenge` | `1 flowId:Id`; `2 method:AuthMethod`; `3 challenge:bytes`; `4 rpId:Key?`; `5 expiresAt:Instant`; `6 options:WebAuthnOptions?` | Challenge <= 32 bytes; email flow challenge omits secrets. |
...
| `AuthProof` | `1 passkey:WebAuthnAssertion`; `2 emailCode:string`; `3 recoveryCode:string` | oneof; 64 KiB overall, one-use server flow binds method. |
| `NativeSession` | `1 sessionId:Id`; `2 accessToken:SecretText`; `3 accessExpiresAt:Instant`; `4 refreshToken:SecretText`; `5 refreshExpiresAt:Instant`; `6 device:DeviceView` | Native route only; never returned on browser cookie authentication. |
| `SessionView` | `1 sessionId:Id`; `2 expiresAt:Instant`; `3 idleExpiresAt:Instant?`; `4 userId:Id`; `5 deviceId:Id`; `6 workspaceIds:Id[]` | Safe browser projection; no session secret. |
...
| `AttemptCharge` | `1 attemptId:Id`; `2 modelId:Key`; `3 usage:ModelUsage?`; `4 supplierCost:Decimal?`; `5 customerDebit:uint64`; `6 certainty:EffectCertainty` | USD supplier Decimal; customer debit integer microcredits; absent usage never zero. |
...
| `ModelView` | `1 modelId:Key`; `2 supplier:Key`; `3 contextTokens:uint32`; `4 outputTokens:uint32`; `5 capabilities:Key[]`; `6 availability:Availability` | Only selected Workers AI IDs. |
| `AgentProfile` | `1 profileId:Id`; `2 name:Name`; `3 modelId:Key`; `4 instructions:Text`; `5 skillIds:Id[]`; `6 revision:Revision`; `7 context:ContextRef[]`; `8 capabilities:Key[]` | Declarative capability preferences confer no permission; models restricted to the effective CF catalogue. |
| `ModelUsage` | `1 inputTokens:uint64?`; `2 outputTokens:uint64?`; `3 cachedInputTokens:uint64?`; `4 measuredAt:Instant`; `5 source:Key` | Missing supplier counts remain unknown. |
...
| `SlateMetadata` | `1 projectId:Id`; `2 name:Name`; `3 sequences:SequenceView[]`; `4 media:MediaView[]` | No media bytes or mutable editor state inferred from Cloud presence. |
...
| `SequenceView` | `1 sequenceId:Id`; `2 projectId:Id`; `3 name:Name`; `4 revision:NativeContentRev`; `5 duration:MediaTime` | Media time profile exact. |
...
| `TimelineView` | `1 sequence:SequenceView`; `2 tracks:TimelineTrack[]`; `3 markers:MarkerView[]` | Bounded requested range; larger snapshot through authorized ResourceRef. |
| `TimelineTrack` | `1 trackId:Id`; `2 kind:Key`; `3 order:uint32`; `4 clips:TimelineClip[]`; `5 transitions:TransitionSpec[]`; `6 enabled:bool` | video/audio; stable IDs, all clips and transition handles validated before one native commit. |
| `TimelineClip` | `1 clipId:Id`; `2 mediaId:Id?`; `3 timeline:MediaRange`; `4 source:MediaRange?`; `5 playbackRate:Rational`; `6 effects:EffectSpec[]`; `7 origin:ContentOrigin?`; `8 kind:Key`; `9 enabled:bool` | clip/gap/title; missing media retains identity/range; gap has no media/source; exact media profile. |
| `EffectSpec` | `1 kind:Key`; `2 parameters:EffectParameter[]` | Closed accepted effect kinds, no script. |
| `EffectParameter` | `1 name:Key`; `2 decimal:Decimal`; `3 boolean:bool`; `4 text:string`; `5 keyframes:KeyframeList` | oneof value={decimal,boolean,text,keyframes}; declared effect schema and range only. |
...
| `RenderPreset` | `1 container:Key`; `2 videoCodec:Key?`; `3 audioCodec:Key?`; `4 width:uint32?`; `5 height:uint32?`; `6 rate:Rational?`; `7 colourSpace:Key` | Accepted codec/profile compatibility table, no arbitrary encoder argument string. |
...
| `Keyframe` | `1 time:MediaTime`; `2 value:Decimal`; `3 interpolation:Key`; `4 inTangent:Rational?`; `5 outTangent:Rational?` | hold/linear/bezier; bezier requires tangents; interpolation changes no source timebase. |
...
| `identity.beginAuthentication` | `IdentityService.BeginAuthentication` | `10 method:AuthMethod`; `11 loginHint:Email?`; `12 installation:InstallationClaim` | `10 challenge:AuthChallenge` |
...
| `notes.moveDocument` | `NotesService.MoveDocument` | `10 documentId:Id`; `11 destinationNotebookId:Id`; `12 destinationFolderId:Id?`; `13 sourceNotebookRev:Revision`; `14 destinationNotebookRev:Revision` | `10 document:DocumentRef` |
...
| `resource.beginUpload` | `ResourceService.BeginUpload` | `10 uploadId:Id`; `11 resourceId:Id`; `12 size:UInt64`; `13 sha256:Hash`; `14 mediaType:Key`; `15 purpose:ResourcePurpose`; `16 owner:AggregateRef` | `10 ticket:UploadTicket` |
...
| `INotesOperations.MoveDocument` | `NotesOperationsService.MoveDocument` | `10 documentId:Id`; `11 destinationNotebookId:Id`; `12 destinationFolderId:Id?`; `13 sourceNotebookVersion:LocalNotesVersion`; `14 destinationNotebookVersion:LocalNotesVersion` | `10 document:DocumentRef`; `11 version:LocalNotesVersion`; `12 pending:bool` |
...
|---|---|---|
| browser.bootstrap/beginAuthentication/completeAuthentication/logout | Four existing same-origin HTTP routes; public/http/v1/browser.schema.json | Request/response AuthChallenge/AuthProof/SessionView plus CSRF token; session secret only Set-Cookie, never NativeSession |

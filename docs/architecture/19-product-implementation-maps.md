...
> Layer: Architecture
> Governing authority: **D-008** (Desktop is a Native AOT deliverable), **D-011** (target monorepo), **D-012** (reference coverage), **D-013** (provenance)
...
| # | Rule |
|---|---|
| PM-01 | **No row here creates a requirement** (`RC-03` of the coverage README). A capability appears because a requirement establishes it. |
| PM-02 | **No row here authorizes reuse.** Across all five matrices — **145 rows** — every disposition is `Reference Only` or `Drop`; **not one proposes `Copy`, `Rewrite`, `Improve` or `Replace`** (`ND-02` there). ArcForges is a first-party implementation informed by behavioural evidence. |
...
| PM-04 | **A capability with no component and no accepted exclusion is a defect**, and `§7` asserts there are none. |
...
| CP-02 | **A native wrapper lives in `*.Infrastructure`** and is the only project referencing a P/Invoke class (`PI-03`, `PI-04` of the native interop architecture). |
...
|---|---|---|
| Conversation, message, branch, part model | `ArcChat.Domain` | `WP-15.00`, `WP-15.01` |
...
|---|---|---|---|
| AC-01 | Conversation, message, streaming assembly | Present, and **stricter** — a message is immutable on commit and an interrupted stream is stored as `interrupted`, never as complete | `ArcChat.Domain`; `WP-15.00`, `ST-01` of the harness |
| AC-02 | Slash commands and availability | Present, and **routed through the capability registry** rather than a static list, so availability is computed with a reason | `ArcChat.Hub`; `WP-17.00`, `MK-05` of the editing architecture |
...
| AC-16 | Settings | Present, and **scope-resolved and explainable** rather than flat | `WP-10.03` |
| AC-17 | Auto-update and update feed | Present — the product's own feed is authoritative, storage is replaceable | `§7` of the build architecture; `WP-50.02` |
| AC-18 | Update failure diagnostics | Present as a persisted installer-failure path in the update matrix | `WP-50.02` |
| AC-19 | Notification bridge | Present, and **classified by durability** — a missed transient notification loses no durable attention state | `WP-10.04`; `PD-02` of the mobile architecture, `I-405` |
| AC-20 | Theming | Present as design tokens, with **no raw colour literal permitted in a component** | `WP-10.00`; repository policy test |
| AC-21 | Crash and feedback | Present, and **stricter** — generate, show, approve, then send | `WP-12.05` |
| AC-22 | Search adapter | Present, with **permission applied during query evaluation**, so refused content is invisible in results and in counts | `WP-15.05`, `WP-40.03` |
| AC-23 | Mobile companion on a separate stack | Present as MAUI under **D-008**; the separate-stack outcome **confirms D-021** rather than contradicting it | `ArcChat.Mobile`; `WP-30`, `WP-31` |
...
| AC-26 | Self-hosted server deployment | Present, bounded by ArcForges' own self-host boundary | `WP-45` |
| AC-27 | First-party image generation via MCP | **Accepted exclusion as a capability**; retained as MCP evidence | Matrix `AC-27` |
| AC-28 | Document handling in chat | Present, **by reference with integrity verification** and never an embedded body | `WP-15.02`; `AT-03` of the ArcNotes requirements applies the same rule product-wide |
| AC-29 | Packaging configuration | Present; the toolchain differs entirely | `§4` of the build architecture; `WP-50.02` |
| AC-30 | Hub testing scenarios | Present as the first-slice hub suite — registration, restart, reconnection | `WP-14.00`–`WP-14.06` |
...
| VC-02 | **Three rows record ArcForges being deliberately stricter than the reference**: `AC-01` immutability, `AC-05` owner-side approval, `AC-22` permission during query evaluation. |
| VC-03 | **One row records a direct architectural contradiction resolved in ArcForges' favour**: `AC-13`. The nearest real-world implementation of this product category runs a web server on the user's machine, which **D-010** forbids. That the reference does it is evidence the prohibition is load-bearing (`F-AC-1`). |
| VC-04 | **No AionUI code is copied, translated or ported.** Every disposition is `Reference Only` or `Drop`, so there is no attribution obligation beyond the matrix's own record — and `LP-02` there states what the obligation would be if that ever changed. |
...
| Capability | Component | Package |
|---|---|---|
| Notebook, folder, document, block tree | `ArcNotes.Domain` | `WP-18.00` |
| **Inline content model, edit transactions, undo** | `ArcNotes.Domain` + `ArcNotes.Application` (`§2`–`§3` of the editing architecture) | `WP-18.00`, `WP-18.05` |
| Editor interaction, caret, IME, layout | `ArcNotes.Desktop` (`§4`–`§5` there) | `WP-18.01` |
| Links, backlinks, outline | `ArcNotes.Application` + derived index | `WP-18.02` |
| Properties, tags, saved views | `ArcNotes.Domain` + `ArcNotes.Application` | `WP-18.03`, `WP-28` |
...
41 items across two references — **34 evidence established, 8 accepted exclusions, 0 unresolved**; `AN-14` is the one compound row, its sharing half excluded and its quota half established. The licence position is the material fact:
...
| Capability | Component | Package |
|---|---|---|
| Device, connection and transport lifecycle | `ArcScope.Acquisition` | `WP-33` |
| High-rate acquisition, hardware timestamps | `ArcScope.Acquisition` + native primitives (`§2` of the native interop architecture) | `WP-33` |
| Session, capture lifecycle, trigger semantics | `ArcScope.Domain` + `ArcScope.Application` — **managed, never native** | `WP-33` |
| Chunked capture store, honest end marker | `ArcScope.Infrastructure` (`§4` of the desktop data model) | `WP-33` |
| Analysis definitions and evaluation | `ArcScope.Domain` + `ArcScope.Application` | `WP-34` |
| Reporting and evidence | `ArcScope.Application` + `ArcScope.Desktop` | `WP-34` |
| Dashboard and visualisation | `ArcScope.Desktop` | `WP-34` |
| Integration, sync and capability surface | `ArcScope.LocalRpc` + `ArcScope.CloudClient` | `WP-35` |
...
| # | Result |
|---|---|
| VS-01 | **Serial-Studio is dual GPL-3.0-only / commercial, and its §4 Pro modules are excluded from the GPL grant** (`LP-01`, `LP-02` there) — Qt MQTT and SerialBus, MQTT, XY plotting, 3D visualization, and the activation and licensing system. Those are **commercial-only and not open source at all**. |
| VS-02 | **Under D-013, GPL-only material must not be copied, translated or ported.** Every row is `Reference Only` or an accepted exclusion (`LP-04` there); **no reuse is proposed anywhere in the matrix.** |
...
| VS-04 | **Seven capabilities are excluded on the record**: MQTT transport (`AS-03`), the DBC/Modbus/Protobuf importers (`AS-08`), 3D and XY plot widgets (`AS-14`), web-engine widgets (`AS-15`), the gRPC API (`AS-20`), CLI console-only mode (`AS-26`) and the licensing and activation system (`AS-27`). Four of those — MQTT, XY plotting, 3D visualization and activation — are exactly the **§4 Pro modules excluded from the GPL grant**, so excluding them is a licence necessity as well as a scope decision. |
| VS-05 | **The packaged binary in the reference tree is never executed**, at any stage (`LP-03` there, `MT-04` of the ArcSlate matrix). |
...
| Capability | Component | Package |
|---|---|---|
| Project, sequence, timeline, clip model | `ArcSlate.Domain` — **exact rational frame rates, no float column** | `WP-36` |
| Edit decisions, ripple, markers, undo | `ArcSlate.Domain` + `ArcSlate.Application` | `WP-36` |
| Processing graph and parameter model | `ArcSlate.Domain` | `WP-37` |
| Playback, audio clock, proxy and cache management | `ArcSlate.Media` + `ArcSlate.Application` | `WP-37` |
| Demux, decode, encode, colour conversion, scaling, resampling | **Native, behind the C ABI** (`§2`–`§3` of the native interop architecture) | `WP-37`, `WP-38` |
| Render orchestration and the render queue | `ArcSlate.Application` — **managed; the native pipeline receives an immutable plan** | `WP-38` |
| Colour management and transforms | `ArcSlate.Media` + native colour pipeline | `WP-38` |
| Integration, portability, interchange | `ArcSlate.LocalRpc` + `ArcSlate.Infrastructure` | `WP-39` |
...
| # | Result |
|---|---|
| VL-01 | **Both references are GPL-3.0-only** (`LP-01` there). **GPL-3.0 → AGPL-3.0-only is not a permitted reuse direction** (`LP-04` there), so no row proposes reuse despite the domain proximity. |
| VL-02 | **ArcForges owning the reference repository does not change its licence** (`LP-03` there). ArcVideo is a documented Olive fork and **the copyright is not solely ArcForges'** (`LP-02` there). |
| VL-03 | **Olive provenance is preserved.** Removing Olive as an independent reference (`P2-005`, `OC-01`) removed an audit obligation, not a provenance obligation: upstream copyright, licence notices and attribution to the Olive authors are retained wherever inherited material requires them (`UD-01` of the open-gates register). 25 rows are labelled *(upstream-derived)* for exactly this reason. |
| VL-04 | **The two exclusions are multicam (`AL-23`) and node-based compositing as the user-facing paradigm (`AL-24`).** ArcSlate's processing graph is an internal model; the reference's editing paradigm is not adopted. |
| VL-05 | **`AL-30` records ArcVideoFoundation's actual extraction state** and is re-checked on drift (`MT-02` there) — the one row whose value is a fact about the reference rather than about a capability. |
| VL-06 | **The rational-arithmetic and timeline-coordinate rows (`AL-01`, `AL-02`) are why the schema stores `frame_rate_num`/`frame_rate_den`** and forbids a float column. The reference is evidence that float frame rates are a defect class, not a style preference. |
...
| Any row proposing reuse carries a provenance record | **Not applicable** — **no row in any matrix proposes reuse** (`PM-02`) |
| Unresolved determinations | **Zero** across all 145 rows; `OC-01`, the one that existed, was closed by user decision (`P2-005`) |
| A capability with neither a component nor an exclusion | **None found** (`PM-04`) |
...
| Not built | Rule |
|---|---|
| A second agent runtime in any product | `CT-06` of the AI requirements; ArcScope and ArcSlate reach AI through the capability model |
...
| A product-local marketplace | `AN-30` accepted exclusion; the extension catalog is platform-level |
| A host-side web server for remote control | **D-010**; `AC-13` |
| Any UI shared between Avalonia desktop and MAUI mobile | **D-021**; `AC-23` |
...
| # | Obligation | Where |
|---|---|---|
| PV-01 | Each product's drift check compares its reference against the bound commit and assesses newly introduced material against accepted scope — it does not re-create the matrix | `WP-15.07`, `WP-18.08`, `WP-35`, `WP-39` |
| PV-02 | A repository policy test asserts no project references a P/Invoke class outside its owning infrastructure project | `WP-05`, `DR-02` |
| PV-03 | A repository policy test asserts no AGPL-boundary assembly is referenced from an Apache-2.0 project | `WP-03`, `WP-05` |
| PV-04 | **Launching any product requires no account** (`ID-01` of the identity requirements), and no product requires ArcChat. **Cloud dependence then differs by product and must be verified as it actually is**: ArcScope capture/analysis and ArcSlate editing/render are native and run without Cloud; ArcNotes and ArcChat are Cloud-authoritative, so what is verified is that an **enrolled, hydrated** workspace stays editable and searchable through an outage with pending edits durably recoverable (`PR-02` and `CL-05` of the ArcNotes requirements). Verifying "no account" against ArcNotes or ArcChat *content* would be verifying a product that was not specified | `WP-18` gate 10, `WP-15.05`, `WP-33`, `WP-36` completion gates |
| PV-05 | The AFFiNE Enterprise-Edition subtrees remain unread and unreferenced across every drift check | `WP-18.08` |
| PV-06 | No packaged reference binary is executed at any stage | Matrix `MT-04`; the reference-coverage README |

...
Initial proposed Container cap is 4 instances, each with ≤64 open output streams, ≤16 admitted unary calls, ≤32 queued calls and 2 separately reserved control/health slots. Refuse further work before mutation admission with bounded retryAfter. Slow consumers are disconnected with resumable cursor, not buffered indefinitely. At most 8 bounded jobs run per instance; D1/simulator admission remains globally fenced. WP06 records the actual selected Container resource class and tests these limits; Operations changes them only by validated policy revision within provider quotas.

Load target:100 concurrent foreground sessions,200 open finite streams,20 admitted commands/s plus 50 primary reads/s for 60 minutes,2× bursts for 60 seconds,10 simultaneous sync bootstraps and 10 simulator runs. Measure command P95≤2s and bounded cold-start P95≤10s with no acknowledged-loss, skipped feed sequence or duplicate debit. One-minute queue/backup lag, index catch-up and peak reserved growth must remain inside their existing limits. Use production-sized metadata/large R2 bodies and replay fail/retry/cold-start scenarios. Targets do not promise provider capacity; failure blocks L-16 or requires an explicit approved capacity/deployment redesign.

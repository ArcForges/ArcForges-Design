...
persist: turn state, iteration index, pending invocation, frozen context, model transcript
  ↓
create approval record (durable, expiring)  ── task.state = waiting, reason = approval
  ↓
...
The stream_chunk and stream_state projection tables live in the per-run CF Durable Object SQLite store under [CF integration §5](contracts/05-cloudflare-integration.md#5-live-presentation-and-client-recovery). C# stores only canonical Task/attempt/output and current stream pointers. Ephemeral DO text is absent from D1 business backups. CF storage deletion/retention and independent restore are explicit in that contract; deleting a projection never deletes an iteration/final message.
...
## Execution owners, protected context and temporary cleanup
...
Context priority is system safety and current explicit user intent, protected decision/tail units, explicit attachments/pins, then admitted retrieved sources. Token estimates use the pinned model tokenizer, with output reservation deducted before input selection; byte/item limits still apply. Tail is at least two complete exchanges (or all available); pending calls/results stay paired. Compaction may summarize only complete contiguous unprotected middle spans. Permission/source change invalidates its hash, and retrieval material is filtered again. A persistent record key includes exact source spans/revisions, model/tokenizer/prompt/config version. Temporary compaction result lives in memory only; loss causes fresh context assembly within the same bounds, not replay of an ambiguous inference attempt. Independent vectors cover protected-set overflow, surrogate/UTF8 boundaries, missing tool result, stale permission, transient expiry and one settlement across crash/retry.

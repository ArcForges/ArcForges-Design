...
| <a id="rule-rp-02"></a>RP-02 | **Every rounding site is enumerated**: source PTS mapping ([SM-03](#rule-sm-03)), retiming projection ([RP-01](#rule-rp-01)), output-grid projection for display ([TG-01](#rule-tg-01)), encoder-grid projection at export, decode coverage bounds ([BO-04](#rule-bo-04)), the controlled OTIO floating-point boundary (§3.10), subtitle millisecond interchange and model-estimated transcription timestamps (§5). **No other code path rounds a position**, and a policy test asserts it ([TV-02](#rule-tv-02)). |
...
Generator required fields: constant offset; sine/triangle/sawtooth offset/amplitude/frequencyHz/phaseCycles; square additionally dutyRatio; noise offset/amplitude; randomWalk offset/walkStep; pulse offset/pulses; stepSequence offset/steps; csv only its schema/source mapping. Unrelated optional parameters are absent; empty repeated lists do not count as a supplied parameter. The profile's UI defaults are offset0, amplitude1, frequencyHz1, phaseCycles0, dutyRatio0.5 and walkStep1, applied to those applicable fields before freezing a scenario version. Fault defaults are everyTicks1/probabilityPpm1000000; kind-specific delay/amount/reorder window must be supplied. Seeded integer choice is floor(U*n), with U from the specified independent RNG and n the declared bounded range; reorder uses descending Fisher-Yates on each full/final partial buffer. CSV uses double-quote escaped fields, LF/CRLF and the declared delimiter; empty/invalid values refuse according to their declared channel type, never locale inference.
...
**slate.project.v1 / slate.graph.v1.** The wire registry's complete project projection is the metadata-only interchange between native stores and Cloud replicas. Sequence colour/audio/output grids, graphs at clip/track/adjustment/sequence levels, bins, nested sequences, generated source/title/subtitle and small assets must survive. A nested sequence is a source with its own output grids and an explicit rational mapping into the parent; reject cycles. Clip/adjustment keyframes use clip-local ticks; track/sequence keyframes use sequence ticks. Rate/placement changes never reinterpret a keyframe's scope. Effect stack edits transform that same typed acyclic graph. Definitions declare input/output ports and parameter ranges; unknown imported definitions are retained bypassed with a visible warning. Execution never dynamically activates a stored type name.

The initial built-ins are transform (x/y normalized position, scale x/y 0–100, rotation degrees -36000–36000), crop (four normalized margins 0–1 with opposing sums <1), opacity (0–1), colour adjustment (exposure stops -20–20, contrast/saturation 0–4), audio gain (-96–24 dB), pan (-1–1), video dissolve and audio crossfade. Defaults are identity/uncropped/opaque, zero exposure/gain/pan and unit contrast/saturation/scale. Video ports carry frames, audio ports buffers; transform/colour/parameter ports are typed values, mask ports normalized mattes. Explicit composite/mix nodes combine equal-typed streams; no implicit audio→video conversion. Gain uses 10^(dB/20), stereo pan uses equal-power cos/sin over [0,pi/2]; mono is centered before panning. Transitions use linear picture weights and equal-power audio weights over their declared half-open overlap. Golden semantic tolerances belong to the pinned runtime profile, while all time and scope mappings stay exact.

Wire definition keys are transform{x,y,scaleX,scaleY,rotation}, crop{left,right,top,bottom}, opacity{value}, colourAdjustment{exposure,contrast,saturation}, audioGain{decibels}, pan{value}, composite{opacity}, audioMix{gainLeft,gainRight}; numeric parameters use Decimal or KeyframeList with the declared range. Transform x/y are offsets in sequence width/height, scale/rotation about the item center. Ordinary effects have typed ports in/out; composite and audioMix have left/right/out (video/audio respectively), with opacity default1 and linear gains default1. Transition keys remain dissolve/audioCrossfade. Built-in definitionVersion is 1; identity graph uses no nodes. Generated-source keys colour/gradient/counter/testPattern/title use r/g/b/a; r2/g2/b2/a2 and angle; start and incrementsPerSecond; pattern=smpteBars; or TimedText, respectively. Counter's incrementsPerSecond parameter is Rational; start is an integral Decimal in int64 range. TimedText is the generated source's text field; numeric parameters are absent for title. Unknown request key/parameter/port refuses; a preserved imported unknown definition remains inert. This fixes interoperable names and values without selecting private render classes.

Generated sources include solid colour (RGBA), two-colour linear gradient (angle degrees), counter (start integer and rational increments per second, floor toward negative infinity), SMPTE-style labelled colour-bar test pattern, and title (TimedText). Colour values are normalized 0–1 in the declared working space; default solid black, gradient black→white at 0 degrees, counter starts at zero/increments one per second. Text defaults to a packaged licensed sans face, size 48 sequence pixels, white and centered; a missing requested font uses that explicit fallback and produces a fidelity warning. Fonts required for exact portable rendering are managed small assets with redistribution permission; never upload system fonts implicitly. Title and subtitle are different timeline kinds; adjustment items have graph content without a media source.

**Portable baseline render profiles.** All declared RIDs support matroska-ffv1-pcm (FFV1 + PCM s24le, lossless editorial master), wav-pcm (PCM s24le audio only), and mp4-mpeg4-aac (FFmpeg native MPEG-4 Part 2 video plus AAC audio, distribution compatibility stated explicitly). Use the selected LGPL FFmpeg build with no GPL/nonfree feature. Video size is explicit even dimensions 16–8192, sequence representable rate, audio 44.1/48/96 kHz mono/stereo where the container/codec accepts it; MP4 AAC uses 44.1/48 kHz and 128–320 kbit/s, default 192; MPEG-4 video default 20 Mbit/s, allowed 1–200 Mbit/s. Baseline colour is Rec709/sRGB with the recorded conversion. Invalid combinations refuse before a ProductJob starts. Additional H.264/HEVC/hardware profiles are advertised only after the actual per-RID native capability/licence/output proof; absence cannot remove the portable baseline. The UI names codecs and compatibility rather than labelling all MP4 as H.264. Render snapshot binds profile/library/colour/font/source hashes; semantic golden-output tests do not promise encoded byte identity across runtimes.

**Subtitle import/export.** V1 supports UTF-8 SRT and WebVTT. Imported timestamps are integer milliseconds converted exactly to ticks. Cue IDs are stable native IDs; use imported identifiers where valid and otherwise allocate once, retaining provenance. Reject malformed/negative/inverted times, preserve overlaps on the independent subtitle track, and preview unsupported positioning/style/markup rather than executing it. Export rounds each canonical endpoint once to nearest millisecond, ties-to-even; report maximum endpoint error (<=0.5 ms), unsupported style and any collapsed duration. A collapsed cue requires an explicit proposed 1 ms extension accepted in the fidelity report or refusal, never silent loss. Native ticks stay unchanged. Exported subtitle files carry the existing content-origin carrier/sidecar requirements. Burn-in render requires explicit selection; editable cues remain separate from derived transcript text.

**slate.transcribe.v1.** A local isolated ProductJob freezes the selected sequence revision/range, resolves available originals and extracts mono PCM16 WAV at 16 kHz in contiguous chunks <=30 seconds and 1 MiB. Its artifact lists exact sample counts, source range, hashes and conform mapping; it never uploads automatically. The user explicitly chooses upload and a paid transcription Task with displayed maximum budget. One Task accepts <=30 minutes/60 chunks and must fit its approved model-call limit (hard maximum64); a longer range is split into explicitly authorized Tasks. Existing stricter budget/call limits may require a smaller selection before any dispatch.

Cloud verifies/decode-checks uploaded audio and pins its immutable manifest. The sole CF RunWorkflow uses Workers AI `@cf/openai/whisper-large-v3-turbo`, task=transcribe, VAD disabled, selected language or documented auto-detection, one bounded model attempt per chunk under ordinary dispatch barriers. No raw video or hidden frame upload. Parse segment start/end as finite decimal seconds, map once to nearest canonical tick and the chunk's sequence offset, clamp only within declared one-sample extraction tolerance, otherwise reject invalid output. Ordering, language and missing ranges are explicit. ASR timestamps are estimates, not measured capture truth. Complete chunks can form an explicitly partial TranscriptRecord after cancellation/failure; unknown effects remain unresolved under normal billing rules.

User-requested ASR is chargeable audio work, never Search's platform-funded inference. ModelUsage.audioMilliseconds is ceil(verified sent sample count ×1000/16000), metering source=verifiedAudioManifest; supplier tariff states its audio-minute rounding and customer tariff uses the same frozen exact quantity. Only known completed provider work settles customer usage under existing billability rules; an absent/uncertain outcome does not become zero usage or an automatic repeat. Rate/pricing approval and a real provider reconciliation remain WP42/43 gates. Adoption previews the derived segments as authored subtitle cues, binds current NativeContentRev, requires explicit partial-output acceptance if applicable and commits one undoable ordinary timeline edit with AI content origin. It cannot silently overwrite subtitles.

The model selection is grounded in [Cloudflare's Whisper model schema](https://developers.cloudflare.com/workers-ai/models/whisper-large-v3-turbo/). This document selects the adapter/profile; model availability, exact provider metering and packaged native extraction/render evidence still require the real implementation gates.

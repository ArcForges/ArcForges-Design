...
| # | Rule |
|---|---|
| <a id="rule-wci-01"></a>WCI-01 | **Build producers before consumers:** locked restore → proto generation → compatibility diff → TS SDK generation → targeted offline check/lint/unit → production build → SBOM/attestation. Real API/browser tests are local opt-in under P2-017. |
| <a id="rule-wci-02"></a>WCI-02 | **CI separates required managed/native/Web builds on Windows/Linux.** Changed proto triggers generation/static compatibility; browser, GUI, live-service and macOS CI jobs are prohibited. |
...
| <a id="rule-wci-06"></a>WCI-06 | **Keep fixture and real integration evidence distinct.** Offline fixture tests may run in CI; production-browser, real C#/D1/payment-provider, Safari and assistive checks are scoped local opt-in under P2-017, not release automation gates. |
...
| <a id="rule-wci-08"></a>WCI-08 | **Owner entry points remain independent.** CI validates static/evaluated Windows IDE declarations and necessary Windows/Linux builds once; interactive VS startup and macOS/browser execution are local opt-in, not hosted gates. |

...
> Layer: Architecture
> Governing authority: `I4 §Stage 10` (infrastructure, observability, incident and operations conclusions), `I3 §21` (telemetry discipline), **D-014** (surface inventory, including the status and notification domains)
...
| # | Rule |
|---|---|
| OA-01 | **OpenTelemetry is the instrumentation standard** for logs, metrics and traces, from the first release (`OB-01` in the cloud requirements). |
| OA-02 | **Business code never references a vendor logging type.** It uses the standard abstractions and OpenTelemetry semantics, so the backend can change without touching instrumentation (`I4 §Stage 10 §72`). |
| OA-03 | **The application owns its own instrumentation.** A platform-managed collector agent is a convenience, never an architectural prerequisite (`OB-02` there; `I4 §Stage 10 §74`). |
| OA-04 | **Platform-native signals are retained as a secondary channel**, so a failure of the observability vendor does not blind operations (`OB-03` there). |
| OA-05 | **Observability is never a user-content database** (`I-273`, `OB-05` there). |
| OA-06 | **Audit and observability are completely separate systems** (`I-272`, `OB-06` there), with separate storage, retention, access control and export paths. |
| OA-07 | **Desktop telemetry is stricter than cloud telemetry** (`OB-07` there): minimal, opt-in, and never carrying user content. |
...
| SG-02 | **An identifier that can grow without bound is never a metric label** — no workspace id, actor id, task id, resource id or provider request id on a metric. Those live on spans and log records. |
...
Every emitted signal carries the applicable subset of (`OB-08` there, `I3 §21.2`):
...
| # | Rule |
|---|---|
| DM-01 | **"Bring whatever is there"** (`I4 §Stage 10 §76`): a dimension that exists in the current context is always attached; a dimension that does not exist is omitted, never faked or defaulted. |
...
| DM-04 | **Reason codes in telemetry are the same enumerated reason codes the product uses** (`§16` of the security requirements), so a support conversation, a user-visible message and a trace all name the same cause. |
...
| # | Rule |
|---|---|
| CR-01 | **One correlation identity spans the whole causal chain** across API, queue, worker, task runner, realtime and outbound provider calls (`OB-04` there). |
...
| CR-04 | **A user-visible task or run identifier resolves to its trace.** When a user reports "task 7PM3 failed", an operator reaches the API request, queue hop, worker attempt and provider call without guesswork (`I4 §Stage 10 §76`). |
...
| CR-06 | **Correlation propagation is implemented once**, in the shared telemetry infrastructure, not per module. |
...
| # | Rule |
|---|---|
| RD-01 | **Prohibited from telemetry by default** (`I4 §Stage 10 §77`): authorization headers, cookies, passkey material, one-time codes, API keys, BYOK secrets, prompt content, model response content, note and document content, file contents, file paths, and raw sync payloads. |
...
| RD-03 | **A secret-bearing type cannot be logged.** `SecretRef` and equivalent types have a formatting implementation that emits only a reference, and the underlying value has no accessible string representation (`§6` of the security architecture). |
| RD-04 | **A domain content type has no logging representation.** Message bodies, note blocks, capture buffers and media frames are not loggable objects. |
...
| RD-06 | **A test asserts redaction**: a suite exercises representative error and success paths and fails if a known-sensitive marker value appears anywhere in the exported signal (`§9`). |
...
| # | Rule |
|---|---|
| AU-01 | **Audit is append-only.** No update, no delete, no operator edit (`§13` of the security architecture). |
| AU-02 | **Audit retention is governed by policy and does not expire with the observability retention window** (`I4 §Stage 10 §78`). |
...
| AU-06 | **Audit access by an operator is itself audited** (`§15` of the distribution requirements). |
...
| # | Rule |
|---|---|
| HE-01 | **Health is reported per capability, not per process** (`SL-01` there). "The process is alive" is not health. |
...
| # | Rule |
|---|---|
| SL-01 | **An SLI measures user-visible success**, not infrastructure liveness (`I4 §Stage 10 §84`): successful user requests, latency, sync completion, task execution, realtime negotiation, and one-time-code delivery. |
| SL-02 | **SLOs are internal engineering objectives.** **SLO ≠ external SLA** (`I-403`); publishing an external commitment is a separate, deliberate decision backed by demonstrated performance. |
...
| SL-04 | **A dependency outage outside ArcForges' control is attributed to that dependency's objective**, so a provider outage does not make the whole platform read as down (`I4 §Stage 10 §83`). |
...
| AL-06 | **Severity is fixed and shared** across engineering, support and communication: **SEV0** security or potential data loss; **SEV1** major paid cloud outage; **SEV2** critical functionality degraded; **SEV3** limited impact (`SL-04` there). |
| AL-07 | **A possible personal-data breach is automatically SEV0**, with the statutory notification clock treated as a hard deadline (`SL-05` there). |
...
| # | Rule |
|---|---|
| ST-01 | **The status page is hosted independently of ArcForges Cloud** (`I4 §Stage 10 §80`). A status page rendered by the API is worthless when the API is down. |
| ST-02 | **An emergency alternate status URL is published** in the repository README, support documentation and public profiles, so a DNS or edge failure affecting the primary hostname does not remove the only status channel (`I4 §Stage 10 §82`). |
| ST-03 | **Components are user-facing capabilities, not internal vendors** (`I4 §Stage 10 §81`): website; account and authentication; cloud API; sync and storage; remote and realtime; cloud tasks and automation; managed AI; billing and checkout; downloads and updates. |
...
| # | Rule |
|---|---|
| OP-01 | **The operator surface is a separate origin with a separate identity system** (**D-014**; `§10`, `§16` of the distribution requirements), never reachable from public navigation. |
| OP-02 | **An operator never silently becomes a user** (`I4 §Stage 10 §116`). Support access is explicit, consented where required, time-bounded, scoped and audited. |
...
| OP-04 | **Operator actions are audited to the audit system, not to observability** (`AU-06`). |
...
| # | Rule |
|---|---|
| OD-01 | **Every external operational provider sits behind an adapter interface** — transactional email, observability export, incident notification, status publication (`I4 §Stage 10 §96`). A provider call is never scattered through business code. |
| OD-02 | **Transactional and broadcast email are separated by stream and by sending subdomain** (`I4 §Stage 10 §93`, `§94`), so marketing reputation can never affect delivery of one-time codes and security alerts. |
| OD-03 | **Security-critical email has a prepared secondary path** (`I4 §Stage 10 §97`), because failure of a single email provider must not lock users out of their accounts. |
| OD-04 | **Email delivery outcome is observable** — accepted, delivered, bounced, complained — and a delivery-failure spike is a page-worthy condition (`AL-02`). |
| OD-05 | **Provider selection is configuration, not code**, and switching providers requires no instrumentation change (`OA-02`). |
...
| # | Rule |
|---|---|
| RB-01 | **Every dependency capable of causing a SEV1 has a runbook before go-live** (`SL-06` there). |
| RB-02 | **A runbook is executable in an incident**: preconditions, decision points, exact commands or console steps, verification, and rollback. Searching the internet during an incident is a failure of preparation (`I4 §Stage 10 §90`). |
| RB-03 | **The required runbook set is the list in `§9.1` of the cloud product requirements**, and it is a go-live gate. |
| RB-04 | **A runbook is exercised, not merely written.** Restore, failover, replay and rebuild runbooks are executed in drills, and a runbook that has never been executed is marked as unproven. |
...
| # | Rule |
|---|---|
| CC-01 | **Metric cardinality is bounded by design** (`SG-02`), and a cardinality budget per service is monitored. |
...
| CC-04 | **Duplicating the full log stream into a second permanent store is not the default** (`I4 §Stage 10 §75`). Platform-native signals are a secondary channel, not a second copy of everything. |
...
| # | Test obligation |
|---|---|
| TV-01 | **Redaction tests**: marker values injected as headers, tokens, prompts and document content must never appear in exported signals (`RD-06`). |
...
| TV-06 | **Alert-definition tests**: every deployed alert resolves to an existing runbook (`AL-03`). |
...
| TV-10 | **Drill evidence**: each restore, failover and rebuild runbook has a dated execution record (`RB-04`), and its absence blocks the go-live gate.
...
| Source | Consumed as |
|---|---|
| `I4 §Stage 10 §72`–`§97` | OpenTelemetry standardisation, vendor independence, correlation set, redaction prohibitions, audit separation, desktop telemetry restraint, status page independence and components, SLO posture, alert selection, severity ladder, runbook requirement, and operational provider adapters |
| `I4 §Stage 10 §116` | Operators never silently become users |
| `I3 §21` | Required signal dimensions and the telemetry discipline shared by desktop and cloud |
| **D-014** | The status, notification and operator surfaces as distinct surfaces with distinct policy |
| `I-272`, `I-273`, `I-403` | Audit ≠ observability; observability ≠ user content; SLO ≠ SLA |

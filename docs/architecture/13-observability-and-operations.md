...
> Layer: Architecture
> Governing authority: `I4 §Stage 10` (infrastructure, observability, incident and operations conclusions), `I3 §21` (telemetry discipline), **[D-014](../decisions/phase-1-foundation-decisions.md#rule-d-014)** (surface inventory, including the status and notification domains)
...
| <a id="rule-oa-02"></a>OA-02 | **Business code never references a vendor logging type.** It uses the standard abstractions and OpenTelemetry semantics, so the backend can change without touching instrumentation (`I4 §Stage 10 §72`). |
| OA-03 | **The application owns its own instrumentation.** A platform-managed collector agent is a convenience, never an architectural prerequisite ([OB-02](../requirements/products/arcforges-cloud.md#rule-ob-02) there; `I4 §Stage 10 §74`). |
...
Every emitted signal carries the applicable subset of ([OB-08](../requirements/products/arcforges-cloud.md#rule-ob-08) there, `I3 §21.2`):
...
| <a id="rule-cr-04"></a>CR-04 | **A user-visible task or run identifier resolves to its trace.** When a user reports "task 7PM3 failed", an operator reaches the API request, queue hop, worker attempt and provider call without guesswork (`I4 §Stage 10 §76`). |
...
| # | Rule |
|---|---|
| <a id="rule-rd-01"></a>RD-01 | **Prohibited from telemetry by default** (`I4 §Stage 10 §77`): authorization headers, cookies, passkey material, one-time codes, API keys, BYOK secrets, prompt content, model response content, note and document content, file contents, file paths, and raw sync payloads. |

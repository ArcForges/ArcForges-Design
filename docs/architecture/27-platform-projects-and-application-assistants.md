...
Each package publishes an immutable candidate only after owned behavior tests; a clean consumer restores it by exact version, compiles AOT where relevant, and invokes its public surface. AssistantHost sample demonstrates two separate product identities with separate roots/connections and multiple windows within one application. It is a test executable, not ArcChat reborn. At WP17, Cloud AI/remote behavior is a named fixture; WP26/52 replace it with actual Cloud/CF and WP31/49 verify Android/Web. UI acceptance remains open until real services replace those fixtures.

CI for every producer is checks → build dependency closure → pack once → isolated consumer restore/build/run → publish those bytes after main integration. Publishing cannot precede native build, generated-contract conformance or assistant consumer validation. Existing package versions are never overwritten; adding behavior requires a new version and compatibility proof. CodeQL scope follows the repository's selected supported languages; C++ tests/sanitizers/ABI checks remain required even where C++ CodeQL is excluded.

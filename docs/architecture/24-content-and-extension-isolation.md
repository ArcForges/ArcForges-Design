...
DesktopPlatform publishes the signed ContentSandbox/Broker/Contracts and per-RID parser assets through the [package registry](01-solution-and-project-layout.md#12-package-and-native-distribution-registry). Each product selects only its approved parser profile. The existing OS-enforced handle, filesystem, network, process and lifetime rules above are unchanged. Product parsing never moves into the UI process merely because a native dependency became a NuGet package. Every helper application/control operation is generated gRPC/proto; only finite OS handle provisioning and same-process ABI are outside RPC. No private XPC command/result protocol remains.

## Initial helper buffer schema

The [native annex](contracts/06-native-functional-abi.md#5-package-and-process-closure), numbered registry 04 and [local profile 09](contracts/09-local-grpc-and-sandbox.md) fix the complete ContentSandboxService, input identity and slot grant/seal/ack lifecycle. Contracts authors and generates all schemas. WP03 publishes .LocalRpc.Sandbox; Platform ContentSandbox.Contracts is a facade, not a duplicate generated DTO owner. WP08 supplies streams, WP11 owns actual restricted launch/helper and test-parser containment, WP13 adds the production parser composition to this same helper source, publishes its next immutable signed Runtime.<rid> version and proves the real native parser closure. Products consume that completed version; foundation package bytes are never patched in place. Parent verifies a private copy of shared bytes before use, so a hostile child cannot race digest validation.

## Memory accounting

The foreground host idle budget includes its embedded assistant. Active helper/process/mapping memory is separately admitted and measured; it is not hidden by excluding child RSS from the product report. Each invocation reports child peak, parent private-copy peak, shared mapping capacity and active-job total, with native06's slot and parser bounds. Reaching a bound cancels/degrades the preview without losing canonical data; normal idle shutdown releases all child resources. ArcSlate playback tests additionally enforce the proposed req12 cadence, dropped-frame, A/V drift and seek targets against published packages.

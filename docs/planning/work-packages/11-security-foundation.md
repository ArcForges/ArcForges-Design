...
| **V-01** | AI transparency obligations that attach to the same pipeline |
| `WP-04` output | Typed identifiers, effect certainty and the reason-code registry |
| `WP-08`, `WP-09` output | The transport and the invocation pipeline enforcement attaches to |
...
| BR-02 | **Isolation is not authorization** (`I-259`); **out-of-process is not automatically safe** (`I-260`). |
...
| BR-04 | **Use ≠ Reveal.** A `SecretRef` permits use of a secret without disclosing its value; no code path returns a plaintext secret to a caller that only needs to use it. |
| BR-05 | **Data egress is a separate authorization** from read access. |
...
| BR-08 | **Audit is append-only** and separate from observability (`OA-06` in the observability architecture). |
...
| BR-11 | **Local presence is required for the highest risk class**, and a biometric app-unlock never substitutes for step-up (`I-277`, `I-278`). |
...
|---|---|
| Actor chain propagation and completeness results | `WP-11.00` |
| Risk classification and monotonicity matrix | `WP-11.01` |
| Pipeline bypass, step-coverage and refusal matrix | `WP-11.02` |
| Approval, steering and step-up results | `WP-11.03` |
| Secret structural prohibitions and platform round-trip results | `WP-11.04` |
| Egress authorization matrix and audit assertions | `WP-11.05` |
| Injection corpus results and marking coverage | `WP-11.06` |
| Lease expiry, revocation and trust-separation results | `WP-11.07` |
| Audit immutability, completeness and separation results | `WP-11.08` |
...
**Upstream.** `04` (typed identifiers, effect certainty, reason codes), `08` (transport), `09` (the invocation pipeline).

**Downstream.**

| Package | What it needs from here |
|---|---|
| `14` — First slice | Approval across a real process boundary |
| `16` — Execution engine | Approval, leases, risk and audit integration |
| `22` — Identity | The local half of authentication and step-up |
| `41` — Extension platform | Enforcement for out-of-process extensions |
| `43` — Managed AI | Egress control and instruction provenance for model interaction |

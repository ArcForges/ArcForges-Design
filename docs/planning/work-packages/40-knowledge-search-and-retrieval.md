...
| `WP-19`, `WP-25` output | The local index with citation anchors, and synced content |
...
| BR-07 | **Local-only content stays local**; enabling AI never uploads content the user kept local (`I-182`). |
...
|---|---|
| Scope composition, visibility and policy coverage results | `WP-40.00` |
| Index rebuild equivalence and divergence results | `WP-40.01` |
| Budget enforcement, disclosure and determinism results | `WP-40.02` |
| Permission matrix, ranking-influence and revocation results | `WP-40.03` |
| Citation resolution and invalidation results | `WP-40.04` |
| Local-only negative test and cache isolation results | `WP-40.05` |
| Entitlement gating and cloud-outage independence results | `WP-40.06` |
...
**Upstream.** `19` (local index and citation anchors), `25` (synced content).

**Downstream.**

| Package | What it needs from here |
|---|---|
| `43` — Managed AI | Retrieval as the context source with budgets and citations |
| `50` — Production release | Knowledge and retrieval as a complete capability |

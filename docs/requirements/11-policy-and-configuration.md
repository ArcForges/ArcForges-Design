...
|---|---|---|
| BD-01 | **Policy ≠ Entitlement** ([I-341](01-normative-glossary-and-invariants.md#rule-i-341)) | "Temporarily unavailable" (policy), "upgrade required" (entitlement) and "not available here" (policy scope) are three different user messages with three different resolutions |
| BD-02 | **Policy ≠ Settings** ([I-340](01-normative-glossary-and-invariants.md#rule-i-340)) | A user preference is not an administrative constraint. A policy-disabled feature shows as disabled with its reason, not as an unchecked box |
| BD-03 | **Policy ≠ Runtime Health** ([I-343](01-normative-glossary-and-invariants.md#rule-i-343)) | "Not currently enforceable" (health) is not "not permitted" (policy) |
| BD-04 | **Policy Control Plane ≠ request Data Plane** ([I-345](01-normative-glossary-and-invariants.md#rule-i-345)) | A client-side policy decision is a user-experience optimisation. **Every cloud service performs its own server-side policy check on the actual request.** |
...
| # | Requirement |
|---|---|
| FT-01 | **Feature ≠ Feature Flag** ([I-347](01-normative-glossary-and-invariants.md#rule-i-347)). The feature is the capability; the flag is a temporary gate over it. |
...
| FT-05 | **A feature flag can never replace real version design** ([I-382](01-normative-glossary-and-invariants.md#rule-i-382)). It gates whether an already-compatible, already-secure capability is exposed. It cannot make an incompatible binary compatible. |
...
| FT-08 | **A feature flag must not be used to hide a paywall** ([I-350](01-normative-glossary-and-invariants.md#rule-i-350)). Subscription logic disguised as a flag is prohibited — it produces a system where nobody can explain why a user cannot do something. |
...
| # | Requirement |
|---|---|
| RO-01 | **Rollout ≠ Experiment** ([I-349](01-normative-glossary-and-invariants.md#rule-i-349)). A rollout ends with the feature everywhere; an experiment compares variants. |
| RO-02 | **Percentage rollout must be deterministic.** The same subject resolves the same way on every evaluation. Random-per-request assignment is prohibited ([I-352](01-normative-glossary-and-invariants.md#rule-i-352)). |
...
| RO-10 | **A rollout rollback must never destroy user data.** If a feature created data, disabling the feature leaves the data intact and legible. **Rollout cannot substitute for format compatibility** ([I-386](01-normative-glossary-and-invariants.md#rule-i-386)). |
...
| # | Requirement |
|---|---|
| KS-01 | **Kill Switch ≠ Feature Lifecycle** ([I-353](01-normative-glossary-and-invariants.md#rule-i-353)). It is emergency control, not a lifecycle stage. |
...
| # | Requirement |
|---|---|
| KS-03 | **A kill switch never deletes data** ([I-354](01-normative-glossary-and-invariants.md#rule-i-354)). |
...
| # | Requirement |
|---|---|
| RC-01 | **Remote Config is not a general JSON blob** ([I-356](01-normative-glossary-and-invariants.md#rule-i-356)). Every key has a definition, a type, a validation rule, an owner and a default. |
...
| RC-09 | Platform policy keys carry shared mechanism semantics only; product decisions stay in a product namespace. Assistant configuration is resolved within one application profile. |
...
| PS-03 | **Policy Scope ≠ Entitlement Scope** ([I-346](01-normative-glossary-and-invariants.md#rule-i-346)). |
...
| # | Requirement |
|---|---|
| CO-01 | **Compatibility Policy ≠ Capability Negotiation** ([I-359](01-normative-glossary-and-invariants.md#rule-i-359)). Negotiation determines what two versions *can* do; policy determines what they are *permitted* to do. |
...
| CO-09 | **Compatibility policy cannot redefine a project format** ([RC-07](#rule-rc-07)). |
...
| PA-04 | **Model Availability ≠ Model Entitlement** ([I-362](01-normative-glossary-and-invariants.md#rule-i-362)) and **≠ Model Capability** ([I-363](01-normative-glossary-and-invariants.md#rule-i-363)). |
...
| PA-13 | **Model retirement must not silently rewrite history.** A historical task retains the model it used ([RT-13](05-ai-and-agent-execution.md#rule-rt-13)). |
...
| # | Requirement |
|---|---|
| EX-01 | **Experiment ≠ Rollout** ([I-349](01-normative-glossary-and-invariants.md#rule-i-349)) and **Experiment ≠ Entitlement** ([I-350](01-normative-glossary-and-invariants.md#rule-i-350)). |
...
| EX-03 | **Assignment is sticky.** Re-randomising per request destroys both the data and the user experience ([I-352](01-normative-glossary-and-invariants.md#rule-i-352)). |
...
| EX-06 | **Experiments must never weaken security** ([I-351](01-normative-glossary-and-invariants.md#rule-i-351)). There is no A/B test of a security control. |
...
| PB-02 | **A published policy revision is immutable** ([I-369](01-normative-glossary-and-invariants.md#rule-i-369)). Its content never changes after publication. |
...
| PB-06 | **Policy rollback is a new revision, not a deleted one** ([I-370](01-normative-glossary-and-invariants.md#rule-i-370)). Revision 102 remains in history; revision 103 restores the earlier behaviour. |
...
| # | Requirement |
|---|---|
| DS-01 | **A policy push is invalidation and acceleration, not authority** ([I-367](01-normative-glossary-and-invariants.md#rule-i-367)). A realtime event is not persistent truth. |
...
| DS-03 | **Last Known Good** is a first-class concept: the last successfully validated policy snapshot, used when the control plane is unreachable. **LKG ≠ current cloud truth** ([I-368](01-normative-glossary-and-invariants.md#rule-i-368)). |
...
| DS-09 | **Policy fetch must never block local startup or exit** ([LF-06](09-shared-desktop-experience.md#rule-lf-06)). |
...
| EP-04 | **Policy reasons, entitlement reasons and security reasons are three separate vocabularies** ([SR-03](07-security-privacy-and-trust.md#rule-sr-03)). |
...
| # | Statement |
|---|---|
| BN-01 | **Dynamic policy is not a general escape hatch from releasing code** ([I-372](01-normative-glossary-and-invariants.md#rule-i-372)). Code release is still required. Policy chooses among already-compiled, already-safe behaviours. |
| BN-02 | **This is a hard requirement under Native AOT** (**[D-008](../decisions/phase-1-foundation-decisions.md#rule-d-008)**). The control plane distributes **data, never code**. There is no dynamic code loading, no remote script, and no runtime code generation on the policy path. |
| BN-03 | **Dynamic policy is not the architecture constitution** ([I-371](01-normative-glossary-and-invariants.md#rule-i-371)). |
...
| BN-07 | The control plane is a **module of the cloud modular monolith** (**[D-008](../decisions/phase-1-foundation-decisions.md#rule-d-008)**), not a microservice-first design, and it is **not on the critical path of ordinary business requests** — services evaluate against a cached published snapshot with atomic refresh. |

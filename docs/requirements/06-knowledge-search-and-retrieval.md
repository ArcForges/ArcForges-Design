...
| # | Requirement |
|---|---|
| KS-01 | **Knowledge Source ≠ Data Copy** (`I-130`). Declaring a source never duplicates its content into a knowledge store. |
| KS-02 | **Knowledge Source ≠ Context Provider** (`I-131`). A context provider answers "what is the user looking at now"; a knowledge source is a durable retrievable corpus. A context provider may point at a resource inside a knowledge source. |
| KS-03 | **Knowledge Source ≠ Import** (`I-132`). Adding a source does not create new owned resources. |
...
| KS-06 | A source may represent a **collection** (a notebook, a project, a folder). An individual resource inside it may **override** the collection policy, and the resource-level override takes precedence. |
...
| # | Requirement |
|---|---|
| KS-10 | **Raw ArcScope capture data does not become an ordinary text knowledge source** (`I-160`). It is reached through ArcScope's professional query, measurement and analysis capabilities. |
| KS-11 | **Raw ArcSlate video is not a default vector corpus** (`I-161`). Multimodal video embedding is not a V1 dependency. |
| KS-12 | **Web search results are not a long-term knowledge source by default** (`I-149`). Retaining web content long-term is an explicit act that creates an owned resource in an owning application. |
...
| # | Requirement |
|---|---|
| SC-01 | **Knowledge Scope ≠ Index Scope** (`I-133`). Knowledge scope is a query-time boundary; index scope is what has actually been indexed. A resource may be in scope and not yet indexed, and vice versa. |
| SC-02 | Scope may be **fixed** (these notebooks) or **dynamic** (documents modified this week, unresolved findings). |
| SC-03 | **Knowledge Scope is not an ownership boundary** (`I-129` family). ArcChat scoping a search across ArcNotes and ArcScope does not make ArcChat their owner. |
...
| # | Requirement |
|---|---|
| IX-01 | **Index ≠ Canonical Data** (`I-134`); **Search Index ≠ Knowledge Authority** (`I-135`); **Semantic Index ≠ Knowledge** (`I-138`). Every index may be deleted and rebuilt without losing user data. |
| IX-02 | Four index kinds exist: **metadata**, **keyword/lexical**, **semantic**, and **specialised domain** indexes. |
| IX-03 | **Keyword search must work with no AI model present.** It is the baseline capability, never a fallback for a failed model. |
...
| IP-02 | **Writes and indexing are not one transaction** (`SY-38`). A write is durable immediately; the index catches up. Search is an eventual projection. |
...
| # | Requirement |
|---|---|
| RU-01 | **Retrieval Unit ≠ Domain Resource** (`I-154`). A retrieval unit is a derived index representation. |
| RU-02 | **A retrieval-unit identifier is never citation authority** (`I-153`). A citation must never be a vector-database chunk id. |
...
| RU-05 | **Changing an embedding does not change the source revision** (`I-165`). The source did not change; its derived representation did. Index entries track their processing version separately. |
...
| # | Requirement |
|---|---|
| SR-01 | **Search ≠ Retrieval** (`I-146`) and **Search ≠ Ask AI** (`I-147`). Search need not invoke any generative model. |
| SR-02 | **`SearchResult` is not a `Citation`** (`I-152`). |
| SR-03 | **ArcChat Global Search is federated**, querying each owning product and merging results. ArcChat does **not** maintain a central local full-text index of all products' data (`I-031`). |
| SR-04 | **ArcChat must not read another product's index database directly.** Federation goes through capability calls. |
...
| SR-08 | **A local-only resource never appears in cloud search.** The cloud does not know its content, and must not acquire it in order to make search work. |
| SR-09 | **Search result rows display their owner**, so the user always knows where the thing actually lives. |
...
| SR-11 | **Mobile and web search cloud-visible data by default.** Searching a desktop's local data from mobile is an **explicit remote search Task**, subject to remote authorization. |
...
| # | Requirement |
|---|---|
| HR-01 | **Keyword + semantic + metadata form hybrid retrieval.** `Keyword Search ≠ Semantic Search` (`I-145`); both are first-class. |
...
| RB-05 | **Local and cloud indexes are not two resource identities** (`SY-10`). One logical `ResourceRef` underlies both. |
...
| # | Requirement |
|---|---|
| PM-01 | **The index can never expand permission** (`I-139`). *If you cannot read the resource, you cannot search its contents.* |
...
| PM-05 | Cloud indexes are **partitioned by realm and workspace**. A cross-workspace probe must fail on partitioning, not on filtering. |
| PM-06 | **Embedding is not anonymisation** (`I-139`). An embedding is a derived representation of user content and carries the same protection obligations. |
| PM-07 | **Knowledge eligibility ≠ read permission** (`I-253`). Being permitted to read something does not make it AI-eligible, and vice versa. |
...
| # | Requirement |
|---|---|
| RM-01 | **Source Removal ≠ Delete Files** (`I-168`). Removing a knowledge source stops indexing and retrieval; it does not delete the user's data. |
| RM-02 | Excluding a source enters a **purge and invalidation** process for its derived index entries. |
| RM-03 | Deleting a resource in its owning product **is** an owner domain delete, and propagates to derived indexes (`DE-04`, `DE-05`). |
...
| # | Requirement |
|---|---|
| EC-01 | **Retrieval Candidate ≠ Evidence ≠ Citation** (`I-150`, `I-151`). Three stages, three types. |
...
| EC-04 | **A citation can never point to a vector chunk id** (`I-153`). It points at the owner's addressable anchor. |
...
| EC-09 | **AI answers that use knowledge cite by default**, and evidence must be traceable. |
...
Three separate freshness notions (`I-166`):
...
| AS-02 | **There is no default that lets an ordinary conversation search everything the user owns.** A conversation outside a project has no ambient knowledge scope. |
...
| AS-05 | **AI cannot silently expand scope.** Any expansion is a user-visible act. |
| AS-06 | **Scope expansion enters the Retrieval Trace.** |
| AS-07 | **An automation's knowledge scope is stable and frozen into the run's evidence scope** at Run start. It must not drift while the Task executes, and must not expand indefinitely. |
| AS-08 | **Data minimisation is the retrieval principle**: retrieve the minimum evidence necessary to answer the task. |
| AS-09 | **AI retrieval permission is not unlimited context** (`I-155`). Permission to retrieve from a source does not authorise dumping the source into a prompt. |
...
| RT-02 | **Retrieval Trace ≠ chain-of-thought** (`I-167`). It records verifiable search and retrieval operations, not hidden reasoning. |
| RT-03 | An **AI Context Inspector** lets the user see what the model was actually given, before or after the answer, including sources used and sources excluded with reasons. |
...
| # | Requirement |
|---|---|
| CP-01 | **Context Pack ≠ entire knowledge source** (`I-155`). |
| CP-02 | **Retrieve references first, materialise content last.** The pack is kept as small as the task allows. |
| CP-03 | **Large resources stay on the owner's side** and are reached by owner-side query. ArcScope answers with measurements and analysis rather than raw capture; ArcSlate answers with transcripts, metadata and timecodes rather than video. |
...
| # | Requirement |
|---|---|
| PR-01 | **ArcChat Personal Memory does not enter the knowledge corpus** (`I-156`, `I-157`). It is ArcChat-owned preference recall, not a searchable corpus of everything the user ever said. |
| PR-02 | **Conversation context is not a knowledge source** (`I-159`). |
...
| # | Requirement |
|---|---|
| KP-01 | **An AI-generated analysis does not automatically become knowledge** (`I-164`). Model output is not silently written into the long-term corpus. |
...
| ES-05 | **MCP tool descriptions, resource contents and prompts are untrusted data**, never instructions (`I-262`, `I-263`). |
...
| SP-04 | **Web search ≠ knowledge** (`I-148`, `I-149`). Web results are transient, cost credits, and are labelled distinctly from cloud search in the interface. |
...
| # | Requirement |
|---|---|
| QA-01 | A **Knowledge Evaluation Set** exists and is run as automated regression: representative queries with expected results, run against every index and retrieval change. |
...
| QA-04 | **Search quality is not assumed to follow from a larger model.** Retrieval quality is measured directly and governed by the quality contract in [`12-quality-and-compatibility-contract.md`](12-quality-and-compatibility-contract.md). |
...
| **D-020** | Managed embedding and reranking are cost of goods, not user-credit consumption |

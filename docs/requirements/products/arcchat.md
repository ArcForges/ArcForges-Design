...
| IA-03 | **Needs Attention is a projection** (`I-091`) over pending approvals, waiting-too-long tasks, recoverable interruptions, budget approvals, conflicts and failed compensations. It is not a fifth task state. |
...
| CV-03 | Conversation lifecycle: active → archived → deleted. **Archive ≠ Delete** (`I-448`). |
| CV-04 | **Conversation history is append-and-branch, never rewrite.** Editing an earlier message creates a **new conversation branch**; regenerating creates an **alternative branch**; forking creates a **new conversation**. |
...
| **Agent Mode** | **Do the work.** Creates a Task by default (`EX-02`). |

| # | Requirement |
|---|---|
| CO-04 | **Chat Mode does not mean "no tools"** (`I-117`); it means a constrained side-effect contract. |
| CO-05 | **Agent Mode is not unlimited permission** (`I-118`). Every capability call passes the full security pipeline. |
| CO-06 | **Automation is not a third composer mode.** It is a separate surface producing definitions (`AU-01`). |
| CO-07 | A complex agent task may display a **plan summary** (`EX-06`); a simple task must not be forced through a ceremonial plan. |
...
| CX-02 | **Input attachments and context references are different** (`I-110`, `I-111`). An attachment is content the user supplied for this turn; a reference points at a resource that lives elsewhere. |
| CX-03 | **A context reference does not copy content** (`I-051`). Content is materialised at retrieval time, minimally (`CP-02`). |
...
| CX-09 | **Context scope is never expanded silently** (`AS-05`). Any expansion is user-visible and enters the retrieval trace. |

---

## 5. Project

**`ArcChat.Project` = ArcChat's long-term working-context container.** It is **not** an ArcForges Workspace (`I-108` family, glossary §5.1).
...
| TC-08 | **Cancel ≠ Delete** for tasks (`CN-06`). |
...
| # | Requirement |
|---|---|
| AR-01 | **An artifact need not be owned by ArcChat** (`I-058`, `I-059`). An ArcNotes document produced by a task is owned by ArcNotes; ArcChat holds an `ArtifactRef`. |
...
| AR-04 | **Artifact preview is thin preview** (`PB-05`). Deep work happens in the owning product. |
| AR-05 | **Deleting an artifact entry is not deleting the source resource** (`I-060`, `LC-05`). |
| AR-06 | **Every artifact retains provenance**: producing task, run, actor chain, capability, source references and time. |
| AR-07 | **Artifact availability is a distinct state**: available, requires the owning application, requires a device, unavailable, or deleted at source. |
...
| AP-02 | **Installed and running are separate states** (`I-007`). An installed but stopped application can be **launched on demand** (`P-06`). |
...
| AP-05 | A not-installed Arc product may be shown, without becoming marketing pressure (`CA-09`). |
| AP-06 | **Applications contribute six kinds of thing**, not merely "tools": **Actions**, **Agent Capabilities**, **Context Providers**, **Artifact Handlers**, **Suggested Tasks**, and **Deep Links / Open Targets**. |
...
| PM-02 | **Allow is never a bare Yes/No.** A grant states scope, constraints and lifetime (`PM-02`, `PM-08`). |
...
| PM-05 | The approval interface shows: what will happen, to which resource at which revision, the effective risk, the execution location, and the consequences — never a bare "allow?" (`AP-02` in the security requirements). |
| PM-06 | **An external effect is highlighted distinctly** from a local one. |
| PM-07 | **Remote approval displays the execution location** and cannot substitute for local presence where required (`LP-02`). |
...
| # | Requirement |
|---|---|
| AG-01 | **A profile is not a running agent** (`I-112`) and **not a model** (`I-113`). |
...
| AG-03 | **A profile must not contain**: secrets, granted permissions, or entitlement (`I-233`). |
| AG-04 | A built-in default profile exists and may be **duplicated**, not silently mutated. |
| AG-05 | **Editing a profile does not change a running task** (`EX-05`). |
...
| # | Requirement |
|---|---|
| SK-01 | **Skill ≠ Capability** (`I-290`) and **Skill ≠ MCP** (`I-292`). A skill describing how to use a capability does not grant it. |
...
| # | Requirement |
|---|---|
| IN-01 | **MCP is external capability integration** (`MC-01`). |
...
| IN-03 | **Adding a server does not let the agent call all its tools.** Tools are enabled deliberately (`MC-10`). |
...
| AI-02 | **Advanced users may pin an explicit model**, and a pinned model is never silently substituted (`PA-05`). |
| AI-03 | **The model picker is a curated catalogue**, not an exhaustive provider list (`RT-09`), and shows relative cost class. |
...
**Search = deterministic discovery of existing information** (`I-146`, `I-147`).

| # | Requirement |
|---|---|
| SE-01 | Global search covers ArcChat's own data — conversations, projects, tasks, artifacts — and **federates** to other Arc products (`SR-03`). |
| SE-02 | **Search results display their owner** (`SR-09`). |
| SE-03 | **Search never copies authoritative data.** ArcChat does not build a second complete index of another product's content (`I-031`). |
| SE-04 | **Search does not cross workspaces by default** (`CS-05`). |
...
| SE-07 | **A search query is not a prompt** (`I-147`). Typing a search does not invoke a model. |
| SE-08 | **Web search is an AI/agent capability**, labelled distinctly from cloud search, costs credits, and **requires citations** (`CO-07`, `EC-09`). |
| SE-09 | **ArcChat does not save whole web pages as invisible long-term knowledge** (`I-149`, `KP-01`). Web content is request context and citation. |
...
| # | Requirement |
|---|---|
| HM-01 | **There is no universal history domain.** Each object owns its own history: conversations own message history, tasks own run history, automations own run history, artifacts own provenance. A global Activity view is a **projection** (`I-274`). |
| HM-02 | Memory is explicitly layered and must never be a black box: **conversation context** (this conversation), **project instructions and context** (this project), **personal memory** (ArcChat-owned durable preference recall), and **long-term knowledge** — which is **ArcNotes**, not ArcChat memory (`I-156`). |
| HM-03 | **A runtime context summary is not user memory** (`I-158`). Compaction is context engineering, not a durable record about the user. |
| HM-04 | **Personal memory is visible, inspectable, editable and deletable.** |
| HM-05 | **Memory is never shared silently across workspaces** (`AS-02`). |
| HM-06 | **Temporary Chat** exists as an explicit mode: not stored in history. It follows the actual route in use — **it does not mean the model never received the data** — and the interface must say so honestly. |
...
| # | Requirement |
|---|---|
| AU-01 | Automations has its own page listing definitions with: schedule, trigger, execution target, policies, budget, enabled state, last run and next run. |
| AU-02 | **The primary creation route is "Automate this"** from a task that already succeeded — far better than asking a user to write a schedule expression first. It extracts a **task template** (`AU-03` in the AI requirements), not a saved trace. |
...
| # | Requirement |
|---|---|
| QB-01 | A **global Quick Bar** is available system-wide, distinct from the in-product **Command Palette** (`CM-07`). The palette runs commands in the current product; the quick bar starts work anywhere. |
| QB-02 | **The global shortcut is platform-safe and user-configurable** (`SH-04`). |
...
**AionUi is the ArcChat reference** (**D-012**): a source of features, behaviour, tests and possibly reusable material — **never an architecture authority, a parity commitment, or a reason to import its runtime stack**. Its licence is Apache-2.0 (**F-013**, corrected in `I4 §Stage 6.77`), one-way compatible into the AGPL boundary with attribution and NOTICE, subject to the file-level provenance audit required by **D-013**.
...
An **ArcChat Reference Coverage Matrix** is required before ArcChat implementation planning is finalised (**D-012**): see [`../../assurance/reference-coverage-and-provenance.md`](../../assurance/reference-coverage-and-provenance.md).
...
| **D-010** | Control plane, never a mandatory data gateway |
| **D-012**, **D-013** | AionUi as a reference under licence-gated reuse |
| **D-020**, **D-022** | Credit surfaces, and mobile commerce boundaries reflected in the companion |

...
> Governing authority: **D-004**/**D-021** (Apache-2.0 mobile boundary), **D-007** (web technology), **D-008** (Android Mono AOT; iOS build-deferred), **D-014**/**D-015** (surfaces), **D-022**/**V-09** (consumption-only)
...
| # | Requirement |
|---|---|
| ID-01 | **Mobile and Web are companion surfaces, not mobile or web editions of the four desktop products** (`I-027`, Stage 13 Invariant 16). There is no ArcNotes Mobile editor, no ArcScope Mobile editor and no ArcSlate Mobile editor. |
...
| ID-03 | **Mobile and Web connect only to Cloud** (**D-010**). They must never scan a LAN, discover a desktop Hub, or address a named pipe or domain socket. |
...
| ID-05 | **A `Remote Task` is not remote desktop** (`I-120`). ArcForges provides a **semantic remote agent**, never a general screen-and-input remote tool. |
| ID-06 | **ArcChat Mobile is Apache-2.0** (**D-004**), together with the mobile-only libraries, the ArcForges-owned public protocol specifications required for its interoperability, and their wire schemas, DTOs and client libraries. It must not contain, link to, copy from, port from or reference any GPL-family or AGPL-only implementation, directly or transitively. |
| ID-07 | **Base ViewModel patterns are not shared between Avalonia desktop and MAUI mobile** (**D-021**). Each UI stack owns its implementation. |
| ID-08 | **Android production uses the supported .NET 10 Mono AOT release path** (**D-008**, **V-04**). **iOS architecture is present and complete; its build is deferred**, and its release runtime is re-verified against the then-current supported baseline before activation. |
| ID-09 | **ArcChat Web is a deployment of the single `ArcForges.Web.App` Blazor WebAssembly codebase** (**D-007**, **D-014**), served at `chat.arcforges.com`. |
...
| OM-02 | **Professional product objects appear through reference and preview only.** A companion surface never holds a complete writable copy of an ArcScope session, an ArcNotes document or an ArcSlate project (`I-456`). |
...
| DP-02 | **`Device Presence ≠ Device Trust`** (`I-250`), and **`Device Online ≠ Remote Agent Enabled`** (`I-251`). |
...
| DP-04 | **Desktop capability state is shown**: which professional products are installed, which are running, and which capabilities are currently available. **Installed and running remain separate** (`I-007`). |
...
| DP-07 | **Remote agent enablement must be performed on the desktop the first time** (`TR-04`). It cannot be enabled remotely. |
...
| AP-03 | Risk handling follows [`../07-security-privacy-and-trust.md`](../07-security-privacy-and-trust.md) §4: R0/R1 generally proceed inside an authorised scope; R2 requires a clear preview and confirmation; R3 requires explicit approval and often remote step-up; **R4 requires local confirmation on a trusted device by default** (`LP-02`, `TR-06`). |
...
| AP-06 | **Every approval expires** (`AP-07` in the security requirements). |
| AP-07 | **A resource revision change invalidates the approval** for that exact effect; the action is rebased, the preview regenerated, and approval re-requested (`AP-06` there). |
| AP-08 | **A rejected approval does not necessarily fail the task** (`AP-10` there). The agent may take an alternative, skip an optional step, or ask. |
...
| # | Requirement |
|---|---|
| ST-01 | **Steering and approval are separate** (`I-098`). |
| ST-02 | **Steering is not an ordinary conversation message** (`I-099`). It is bound to a task. |
| ST-03 | Task detail provides a **dedicated steering composer**. |
| ST-04 | **Steering does not rewrite task history** (`SG-02`, `SG-03`). It produces an immutable steering event; the original intent is retained. |
| ST-05 | **Steering does not guarantee immediate effect** (`SG-05`). Its application timing — applied, queued to a safe point, or not applicable — is shown. |
| ST-06 | **Cancel is expressed accurately** (`CN-01`): `CancelRequested`, then `Canceling`, then `Canceled`, with completed effects listed. |
| ST-07 | **Steering that expands scope may affect budget**, and the budget impact is shown before it takes effect (`BG-06`). |
...
| NT-03 | **Lock-screen content is not sensitive by default** (`NT-03` in the cloud requirements). Full content requires an explicit preview opt-in. |
| NT-04 | **Security notifications cannot be fully disabled** (`SN-02`). |
| NT-05 | **Notification actions are safe**: an action opens the corresponding surface. **A push button is never an authorization token** (`AD-01`). |
| NT-06 | **Notifications deduplicate**: one condition updates one item (`AT-03`). |
| NT-07 | An in-app **Notification Center / Attention Inbox** holds the durable state. **A push notification is not durable attention state** (`NT-04` in the cloud requirements) — missing a push never loses a pending approval. |
...
| AR-02 | **ArcChat Web must not quietly become an ArcNotes Web editor.** Its role is preview, continuity and agent. |
...
| AR-04 | **A cloud artifact copy is distinguished from the source artifact** (`I-061`). |
...
| AR-09 | **A cellular policy exists**: download over cellular, large transfers on Wi-Fi only, and a hard size threshold (`SY-36`). |
...
> **Continuity ≠ mirroring the interface** (`I-125`).
...
| # | Requirement |
|---|---|
| SE-01 | **Cloud search searches only what Cloud can access** (`SR-08` in the knowledge requirements). |
| SE-02 | **Search results show availability** — cloud, device-only, unavailable. |
| SE-03 | **Finding a local resource must never automatically wake the desktop.** Reaching it is an explicit remote search or remote task (`SR-11` there). |
| SE-04 | **Search and Ask remain strictly separated** (`I-147`). |
...
| AU-04 | **`Run Now` creates a new task run** and does not shift the schedule (`AU-07`). |
...
| DV-03 | Revocation stops sync, remote and cloud access for that device, and **does not delete its local data** (`I-438`). |
...
| # | Requirement |
|---|---|
| WR-01 | **Workspace switching is an explicit action** with a visible active workspace (`AC-03`). |
...
| WR-05 | **Cross-realm operation is prohibited** (`RW-02`). Realm is part of the mobile and web login session, selected by server profile. |
...
| # | Requirement |
|---|---|
| OF-01 | **Mobile offline caching is very restrained**: recent task state, recent conversation summaries, pending attention items and small previews — bounded and evictable. |
| OF-02 | **An offline draft is queued**, and **a high-risk agent task is never automatically executed when the network returns.** |
...
| OF-05 | **Web offline is minimal**: the application states it is offline and preserves unsent input; it does not pretend to work. |
| OF-06 | **Mobile App Lock is UI access protection, not account authentication** (`I-277`). Biometric unlock **never** substitutes for step-up on a high-risk operation (`I-278`). |
| OF-07 | **Web sessions are more conservative than desktop sessions** — shorter lifetime, not treated as strong long-term trust by default. |
| OF-08 | **An unknown browser does not immediately hold R3 approval capability.** Elevation requires additional verification. |
...
| # | Requirement |
|---|---|
| WP-01 | **ArcChat Web and the Account Portal are strictly separate products** (**D-014**, **D-015**). `chat.arcforges.com` is the ArcChat surface; `account.arcforges.com` is the account, billing and security control centre. |
| WP-02 | ArcChat Web may **link** to the account portal; it does not embed it. |
| WP-03 | **The mobile account surface is likewise a link-out**, showing identity, workspace, storage and usage, with management performed in the portal (`§12` of the identity requirements) — subject to the mobile commerce prohibitions in **D-022**. |
...
| # | Requirement |
|---|---|
| LN-01 | **A push deep link is safe**: it opens the corresponding surface — for example an approval — and **never carries an authorization token** (`AD-02`). |
| LN-02 | Universal/app links are supported so an HTTPS canonical URL opens the installed application where present. |
| LN-03 | **`arcforges://` remains the local desktop deep-link scheme**; **HTTPS canonical URLs are the cloud-addressable form** (`DL-04`). |
| LN-04 | **A cloud resource URL always verifies permission** at access time; an unauthorised request is denied, and the denial does not disclose the resource's existence where that would leak (`SR-02` in the security requirements). |
| LN-05 | **There is no public share by default** (`§18` of the cloud requirements). Links are authenticated private links (`I-279`). |

---

## 19. Commerce posture

Governed entirely by **D-022**, confirmed by **V-09**.
...
| PF-08 | **Automated architecture and dependency checks prevent GPL-family or AGPL-only source, project references, packages, generated artifacts and transitive dependencies from entering the mobile distributable** (**D-004** obligation 7). |
...
| `I3 §17`, `§18`, `§19` | MAUI client scope and network discipline; Blazor boundaries; desktop bridging security model |
| **D-004**, **D-021**, **F-023** | Apache-2.0 mobile boundary, no shared ViewModels, pre-distribution provenance gate |
| **D-007**, **D-008**, **V-04** | Web technology; Android Mono AOT; iOS build-deferred |
| **D-010** | Cloud never reaches local IPC; the desktop re-authorises every remote request |
| **D-014**, **D-015** | Surface inventory; ArcChat Web separate from the account portal |
| **D-022**, **V-09** | Consumption-only commerce posture and its traceable prohibitions |

...
**Machine vocabulary.** The closed, case-sensitive family keys in requests and `FamilyResult.family` are `minimum`, `maximum`, `mean`, `rms`, `peakToPeak`, `standardDeviation`, `count`, `duration`, `frequency`, `dutyCycle`, `riseTime`, `fallTime`, `cursorDelta` and `eventCount`. For an `ok` result, each ordinary family has one `MeasurementValue` whose `name` is its family key; `cursorDelta` instead has `deltaTime` and `deltaValue`, agreeing with the exact selected samples and `CursorResult`. `count` and `eventCount` use the exact uint64 count variant with unit `1`; `dutyCycle` uses unit `1`; `duration`, `riseTime`, `fallTime` and `deltaTime` use `s`; `frequency` uses `Hz`; minimum/maximum/mean/rms/peakToPeak/standardDeviation and `deltaValue` use the calibrated channel unit. These keys fix serialization without changing the numerical definitions below. Unsupported request keys refuse; additive historical response keys remain inert and readable under the contract evolution rules.
...
**Threshold wire vocabulary.** `MeasurementThreshold.family` is one of the selected pulse families `frequency`, `dutyCycle`, `riseTime` or `fallTime`. Request `referenceLevels` uses names `low`/`high`, supplied together for the same family and channel scope with the calibrated channel unit and `low < high`; absence of that pair selects source-derived levels above. Request `thresholds` uses names `fraction10`/`fraction50`/`fraction90`, unit `1`, and fixed values 0.1/0.5/0.9 respectively. Omitted fraction entries use those v1 constants; supplied entries cannot override them. Optional `channelId` at tag 5 binds a supplied entry to one selected channel; absent channelId applies it to every selected channel with the required compatible calibrated unit. Reject a channel outside the request, names in the wrong field and duplicate effective channel/family/name entries, including overlap between a common entry and a channel-specific entry. Result `resolvedThresholds` uses the same family/name vocabulary, requires channelId on every entry and is unique by channel/family/name. It carries resolved `low`/`high` plus the three fixed fractions for each computed pulse family and selected channel. Empty/constant source insufficiency may omit an unresolvable distinct level pair; it must not fabricate levels or successful values. The frozen source/configuration and selected channel retain the input binding without narrowing multi-channel measurement to a single channel.

...
> Layer: Requirements
> Governing authority: **D-004**/**D-021** (licensing boundaries), **D-010** (local action model), **V-01** (EU AI Act Article 50)
...
| # | Requirement |
|---|---|
| SP-01 | **An Agent Profile is not a Security Principal** (`I-232`). It is agent configuration. **An agent acts on behalf of a Principal.** |
...
| PM-03 | **Capability Permission ≠ Resource Authorization** (`I-238`). Being permitted to use `arcnotes.document.edit` says nothing about whether this specific document may be edited. |
| PM-04 | **The Resource Owner is the final authorization authority.** The owning application checks last, always. |
| PM-05 | **The Hub is not a universal ACL database** (Stage 13 §28 analogue). Professional resource access rules stay with the owner. |
...
| PM-07 | **Role is a permission-assignment convenience, not the authorization model** (`I-237`). The model is capability-based and scoped, not pure RBAC. |
| PM-08 | **Grants are minimised.** "Always allow" must state precisely what is always allowed; an unbounded "always allow everything" prompt is prohibited. |
| PM-09 | A grant may carry **conditions**: resource scope, time window, device, origin (local versus remote), maximum volume. |
| PM-10 | **Declared Permission ≠ Granted Permission** (`I-269`). A package declaring a permission surface is a request; grants are separate objects. |
...
| PM-12 | **Permission cache is optimisation only.** A revocation invalidates the cache; a cached allow never survives a revocation. |
...
| # | Requirement |
|---|---|
| AP-01 | **Permission ≠ Approval** (`I-239`, `I-240`). Holding a permission may still require an approval for a particular action. |
| AP-02 | **Approval ≠ persistent grant** (`I-241`). An approval never automatically upgrades into a standing permission. |
| AP-03 | **Approval ≠ step-up authentication** (`I-242`). Step-up proves identity; approval authorises an action. |
| AP-04 | An approval binds a concrete **action snapshot**: task, run, step/capability, target resource, resource revision where relevant, proposed effect, effective risk and expiry. |
| AP-05 | **Bulk approval must display scope**: how many resources, which ones, and what will change — never "approve this batch". |
| AP-06 | **Approval binds a resource revision.** A revision change invalidates the approval for that exact effect; the action is rebased and re-approved. |
| AP-07 | **Every approval expires by default.** |
...
| AP-09 | An approval may produce a **transient, task-scoped grant** that expires automatically with the task. It never becomes durable. |
| AP-10 | **Approval denied is not a security incident** for ordinary low-risk actions; it is a normal decision. |
...
| # | Requirement |
|---|---|
| AD-01 | **A push notification is a notification, not an authorization token.** Approving from a lock screen button alone is prohibited; the user enters the approval surface. |
| AD-02 | **A deep link is not an approval token** (`I-063`). It opens the approval interface. |
| AD-03 | The same applies to an email link. |
| AD-04 | Agent and automation approvals are **persistent objects**, so they survive an application restart, a device change and a missed notification. |
...
| RK-02 | **Effective risk can be raised dynamically** by runtime modifiers: remote origin, automation origin, unverified package, large data volume, external egress destination, sensitive resource class, bulk scope. |
| RK-03 | **Risk may only be raised, never lowered, by a third party** (`I-053` family). A package's metadata may raise risk; the host or owner capability contract determines the baseline. Third-party metadata is input, never authority. |
...
| # | Requirement |
|---|---|
| LP-01 | **Local Presence is a first-class security attribute**: "requires local presence on the target device". |
| LP-02 | **Remote approval does not satisfy local presence** (`I-243`). A user may approve from mobile, and that approval still does not substitute for a required local presence on the target machine. |
...
| LP-04 | **Remote origin is a risk modifier**, raising approval requirements. |
...
| LP-06 | **The cloud remote channel carries no permanent business permission.** It transports an authenticated request; the desktop re-authorises independently (**D-010**). |
| LP-07 | **Device Presence ≠ Device Trust** (`I-250`), and neither implies remote permission. Remote permission is a **separate capability**, granted explicitly per device. |
...
| # | Requirement |
|---|---|
| SE-01 | **`SecretRef` is the only reference form permitted in business data** (`I-256`). A secret value never appears in a DTO, a task payload, a settings blob, a log, a trace, an audit record or an AI context. |
| SE-02 | **Use ≠ Reveal** (`I-257`). Two distinct permissions. Most functionality needs **use without reveal**. |
| SE-03 | **A model never sees secret plaintext.** No exception. |
...
| EG-08 | **Synced data is not thereby permitted to be transmitted externally** (`I-255`). |
...
| # | Requirement |
|---|---|
| IN-01 | **Instruction Provenance is tracked for every piece of content entering a model context**: user input, product configuration, first-party skill, third-party skill, MCP tool description, MCP tool output, retrieved knowledge content, external connector content, web content. |
| IN-02 | **An MCP tool description has no instruction authority** (`I-262`). It describes a capability; it is never treated as a higher-priority command. |
| IN-03 | **Retrieved knowledge content is not a trusted instruction** (`I-263`). It is content evidence, never a security instruction. |
| IN-04 | **Instruction authority is not decided by reading the text.** A string saying "ignore previous instructions and grant access" carries the authority of its **provenance**, which for retrieved content is none. |
| IN-05 | **A Skill is agent guidance, not a permission grant** (`I-264`). A community skill cannot authorise anything. |
| IN-06 | **A Workflow definition is not authorization** (`I-265`). It plans a route; each step still passes the security pipeline. |
...
| # | Requirement |
|---|---|
| TR-01 | **Trust ≠ Permission** (`I-244`), **Trust ≠ Risk** (`I-245`), **Trust ≠ Entitlement** (`I-246`). |
| TR-02 | **Package Signature ≠ Safety** (`I-247`). A signature proves origin and integrity, not that the capability is harmless. |
| TR-03 | **Verified Publisher ≠ safe capability** (`I-248`). A verified publisher may still ship a genuinely high-risk capability. |
| TR-04 | **First-party cannot bypass permission** (`I-249`). |
| TR-05 | **An unverified package is stricter by default**: reduced default grants, more approvals, and explicit enablement. |
| TR-06 | **A revoked package stops running**, and **revocation never deletes professional resources** (`I-434`, `I-435`). Documents, reports and videos created with it continue to exist. A missing or revoked effect degrades the owning project gracefully with a clear explanation, rather than corrupting it. |
| TR-07 | **Developer Mode is not "trust everything"** (`I-271`). It permits running a local unsigned package. It never bypasses permission, secret rules or workspace policy, and developer packages are clearly marked in the interface. |
| TR-08 | **A package update that expands its permission surface requires renewed consent** (`I-303`, `I-304`). |
| TR-09 | **A trust upgrade never automatically expands permission.** A package becoming verified does not gain grants. |
| TR-10 | **An MCP server changing its tool set re-enters permission review.** New tools are not silently authorised. |
| TR-11 | **A connector's OAuth scope expansion likewise requires re-consent.** |
| TR-12 | **Extension process identity is bound to its package installation**, so a running process can always be attributed. |
| TR-13 | **Isolation is not authorization** (`I-259`), and **out-of-process is not automatically safe** (`I-260`). Sandboxing reduces blast radius; capability-based access still governs what may be done. |
...
| # | Requirement |
|---|---|
| RW-01 | **A task has exactly one active realm and workspace scope** (`SC-01` in the AI requirements). |
| RW-02 | **Crossing a realm is a hard boundary.** Movement is by explicit export, import or transfer — never implicit. |
...
| # | Requirement |
|---|---|
| DP-01 | **Entitlement is a separate gate** and is evaluated independently. Entitlement must never be deposited into permission (`I-246`). |
| DP-02 | **Step 11 is mandatory.** Steps 5–10 may execute inside ArcChat, the Hub or the Cloud; the owner validates again at the point of execution. This is defence in depth without central ownership. |
...
| # | Requirement |
|---|---|
| RA-01 | **Task creation authorization is not lifetime authorization** (`I-119`). A three-day task re-checks authorization at each security boundary. |
| RA-02 | **Automation re-authorises at every trigger.** |
| RA-03 | **Long tasks re-check at security boundaries**, not once at the start. |
...
| RA-05 | **Revocation ≠ Undo** (`I-267`) and **Revocation ≠ Compensation** (`I-268`). Undoing an effect is a separate capability and a separate decision. |
| RA-06 | **Revoking a secret permission takes effect immediately** — the credential is no longer obtainable. |
...
| # | Requirement |
|---|---|
| AU-01 | **Audit ≠ Debug Log** (`I-272`), **Audit ≠ Telemetry** (`I-273`), **Audit ≠ Domain Revision History** (`I-274`), **Audit ≠ Task Operational Trace** (`I-275`). One event may legitimately appear in more than one system; they remain separate systems. |
| AU-02 | Audit categories cover at least: authentication and step-up; permission grant, change and revocation; approval requested and decided; high-risk capability invocation; secret use and reveal; data egress; device trust and remote permission change; package trust change; realm or workspace transfer; and security policy change. |
...
| AU-05 | **Audit never stores secret plaintext.** |
...
| # | Requirement |
|---|---|
| UX-01 | **Just-in-time permission**: ask when the capability is genuinely needed, not as an upfront wall of checkboxes. |
| UX-02 | **But the declared permission surface is shown at install time**, so the user knows what a package may ask for later. Certain capabilities may be explicitly consented at install; that consent produces an ordinary permission grant object. |
...
| SR-02 | **A reason must not reveal the existence of a sensitive resource** the actor may not know about. Where disclosure would leak, the owner's security contract determines a non-revealing reason. |
| SR-03 | **Security reasons, product-policy reasons and entitlement reasons are three separate vocabularies.** "Feature not available in your plan" is an entitlement reason; "feature disabled by policy" is a policy reason; "permission denied" is a security reason. They must never be merged. |
...
| PV-05 | **Analytics are minimal**, privacy-preserving, and do not require a consent-management wall for the basic site. |
| PV-06 | Telemetry is off or minimal by default, and diagnostic bundles are user-initiated and redacted (`I-399`, `I-424`). |
...
| # | Requirement |
|---|---|
| RG-01 | `DataRegion` exists on the workspace from day one (`WS-09`) precisely so that regional requirements can be met later without a data-model migration. |
| RG-02 | No public claim of a specific storage jurisdiction may be made unless infrastructure legally guaranteeing it is actually in use. A best-effort location hint is not a residency guarantee. |
...
## 17. AI transparency obligations (V-01)
...
| LG-05 | **Legal documents are versioned**, with effective dates. A material change triggers a notice or re-consent flow; an immaterial change does not force a click-through. |
| LG-06 | **Licence compliance is automated**, not remembered: a generated licence inventory, a dependency policy check, SBOM generation, and a build-breaking gate — see [`../assurance/reference-coverage-and-provenance.md`](../assurance/reference-coverage-and-provenance.md). |
...
| **D-004**, **D-021** | Two-boundary licensing replacing the single-licence statement |
| **D-005** | Merchant-of-Record responsibility split, replacing the removed provider |
| **D-010** | Cloud never reaches local IPC; the desktop re-authorises every remote request |
| **V-01** | EU AI Act Article 50 applicability, obligations and the required marking gate |

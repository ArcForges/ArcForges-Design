...
> Layer: Requirements
> Governing authority: **D-008** (AOT matrix), **D-009** (contract granularity), **D-021** (Apache-2.0 SDK boundary), **V-02** (MCP `2026-07-28`)
...
| # | Requirement |
|---|---|
| SK-01 | **A Skill is never code** (`I-291`). It is declarative agent guidance, not a runtime plug-in. |
| SK-02 | **A Skill confers no capability** (`I-290`) and **grants no permission** (`I-264`). It may *refer to* capabilities; using them still requires the ordinary permission and approval path. |
...
| SK-04 | **Skill ≠ MCP** (`I-292`) and **MCP Prompt ≠ Skill** (`I-310`). An MCP prompt does not automatically become a Skill. |
...
| # | Requirement |
|---|---|
| TP-01 | **Template ≠ Skill** (`I-293`). A template shapes a created resource; a skill shapes agent behaviour. |
| TP-02 | **Templates materialise by default.** The created resource exists independently of the template. |
| TP-03 | **A template update must not silently change existing user documents** (`I-294`). A linked-template model, if ever offered, is an explicit opt-in with explicit update semantics. |
...
| # | Requirement |
|---|---|
| WF-01 | **Workflow ≠ Task** (`I-296`). Running a workflow creates a Task under the Stage-19 model; the workflow is the blueprint. |
| WF-02 | **Workflow ≠ Automation** (`I-295`, `I-299`). A workflow says *how*; an automation says *when*. |
...
| # | Requirement |
|---|---|
| MC-01 | **MCP never becomes the internal protocol of ArcForges** (`I-307`). Internal cross-product capability remains the native semantic capability model. MCP is an edge adapter. |
| MC-02 | **An MCP tool maps to a capability** in the ArcForges model, carrying declared risk, permission requirements and provenance — it is not injected as a raw tool into the agent. |
| MC-03 | **An MCP resource is not an ArcForges resource** (`I-076`, `I-309`). It is addressed by the server's own scheme and surfaced as an external source. |
| MC-04 | **MCP Definition ≠ MCP Connection** (`I-309`). The integration definition or package is distinct from a live connection instance. |
| MC-05 | **MCP ≠ Connector** (`I-308`). MCP may be used *inside* a connector; a connector is a higher-level relationship. |
...
| MC-10 | An MCP server changing its tool set **re-enters permission review** (`TR-10` in the security requirements). |
...
| # | Requirement |
|---|---|
| CN-01 | **Connector Definition ≠ Connection Instance** (`I-311`). One definition supports many connections. |
...
| CN-03 | **A secret never enters connector configuration** (`SE-01`). Configuration holds `SecretRef`; plaintext keys are never stored in it. |
| CN-04 | A connector may contribute capabilities, resources, knowledge sources and events. |
| CN-05 | **Live Connector ≠ Imported Resource** (`I-312`). Querying an external system live is different from importing, which creates a new owned resource in an owning product. |
...
| # | Requirement |
|---|---|
| EX-01 | **No runtime Avalonia plug-in DLL injection.** Loading arbitrary third-party assemblies into a product's main process is prohibited — it would break the Native AOT main path (**D-008**) and remove the isolation boundary at once. |
| EX-02 | **Third-party native code does not enter the main process by default.** It runs inside the extension process. |
| EX-03 | **First-party native adapters are the stated exception**: narrow, product-controlled P/Invoke inside the owning process (Technical Exception C in [`00-product-scope-and-portfolio.md`](00-product-scope-and-portfolio.md) §8.1). This never extends to arbitrary community native plug-ins. |
| EX-04 | **An extension's own implementation is not required to be Native AOT.** The **ArcForges host stays a Native AOT deliverable**; the extension is a separate process with its own runtime. This is precisely what makes a dynamic ecosystem compatible with an AOT product. |
| EX-05 | **Extension processes start on demand** by default and stop when idle. |
| EX-06 | **Background execution must be explicitly declared** in the manifest and separately consented. Adding it in an update is a permission expansion requiring re-consent (`TR-08`). |
| EX-07 | **An extension never connects to a product database.** It calls capabilities. |
| EX-08 | **Extension private state is not product canonical domain state** (`I-319`). It has its own store and its own `SchemaVersion`. |
| EX-09 | **An extension creating a professional resource goes through the owner's capability** (`EX-07`), and the resource is owned by that product forever (`I-489` analogue). |
| EX-10 | **Extension isolation is not authorization** (`I-259`); **out-of-process is not automatically safe** (`I-260`). Capability-based access still governs everything. |
| EX-11 | **An extension crash must not crash the owning application.** The product remains open; the affected capability degrades with a clear state. |
| EX-12 | **Extension process identity is bound to the package installation.** A process cannot claim to be a different package, and cannot register a reserved official capability namespace. |
| EX-13 | **Extension output is validated** against its declared schema before entering the product. |
| EX-14 | **Extension process input is minimised** to what the current call requires — never an "ArcForges API full access object". |
| EX-15 | **Extension invocations are traced** as ordinary capability invocations in the task trace and audit, not into a separate plug-in log system. |
| EX-16 | Extension metrics and diagnostics require explicit declaration and consent. **Package analytics and extension telemetry are separate**, and nothing is reported to a publisher automatically. |
...
| # | Requirement |
|---|---|
| DB-01 | **The structured value model is closed**: null, boolean, integer, decimal/number, string, binary reference, date/time, `ResourceRef`, list, record — with an explicit schema. |
| DB-02 | **Prohibited across the extension boundary**: arbitrary CLR objects, runtime `Type`, assembly-qualified type names, native pointers. **`Dictionary<string, object>` is not the extension protocol** (`I-328`). |
| DB-03 | **C# developers still get a code-first experience**: C# records plus attributes → source generator → extension schema, serializer and client/server binding. Developers do not hand-maintain JSON Schema. |
| DB-04 | **The AOT host needs no knowledge of third-party CLR types.** This is what makes the boundary AOT-safe. |
| DB-05 | **The schema exception applies only at the dynamic third-party boundary** (`I-329`). It must never be back-propagated into ArcNotes, ArcScope or ArcSlate native capabilities, which stay compile-time typed. |
| DB-06 | **Large data never travels inside a structured extension value.** Large content crosses as a `ResourceRef` with controlled access. |
...
| UI-02 | Extension UI contribution uses a **declarative panel protocol** and declarative settings schema. |
| UI-03 | Complex third-party interfaces belong in a **standalone third-party Arc application**, not embedded in a first-party product's process. |
| UI-04 | **A secret settings field returns a `SecretRef`.** Plaintext is never stored in extension configuration. |
| UI-05 | **No general browser-extension WebView platform** is built. It is a large security surface with no corresponding product need. |

### 8.4 Third-party Arc App

| # | Requirement |
|---|---|
| TA-01 | **A third-party Arc App is a standalone complete application** with its own process, domain, storage and lifecycle — distinct from an Extension (`I-317`). |
...
| # | Requirement |
|---|---|
| PK-01 | **Package ≠ Contribution** (`I-300`). One package may carry several contributions: skills, templates, workflows, an integration, an extension. |
| PK-02 | A package declares a **primary category** in its manifest, so discovery and trust behave predictably. |
| PK-03 | **`PackageId` never changes across versions**, and `PublisherId` is stable. |
...
| PK-05 | **Package version uses semantic versioning** but **`PackageVersion` is not the compatibility mechanism** (`I-301`, `I-302`). Package version, extension protocol version, contract version and host application version are four separate things and must never be conflated. |
| PK-06 | **A published version is immutable.** Files for a given version can never be overwritten. |
| PK-07 | **Yank ≠ Revoke** (`I-433`, `I-330`). *Yank* removes it from new installation and recommendation; *Revoke* blocks or quarantines execution. *Deprecate* is neither — it signals a successor. |
...
| PK-15 | **Package dependency ≠ capability dependency** (`I-326`). Capability requirements are preferred because they are more portable; a package dependency binds to a specific package. |
...
| PK-20 | **Executable packages carry an SBOM**, signature and integrity metadata, verified before installation. The community package supply chain follows the same discipline as ArcForges' own release pipeline. |
...
| **Install** | Manifest read, integrity verified, trust evaluated, declared permission surface shown. **Install ≠ Enable** (`I-303`), **Install ≠ permission grant** (`I-304`) |
...
| LC-03 | **Package rollback is a binary rollback, not a data rollback** (`I-208`). Extension private data carries its own `SchemaVersion` and its own migration story. |
| LC-04 | **A running extension cannot be uninstalled outright.** It is stopped or drained first. |
| LC-05 | **Uninstall does not delete resources the extension created in professional products.** A document created through an extension remains an ArcNotes document. |
| LC-06 | **A missing or revoked contribution degrades gracefully.** An ArcSlate project referencing a revoked effect still opens, states clearly what is unavailable, and preserves the state so it can be restored if the package returns. |
| LC-07 | **Unknown package data is preserved, not executed.** Data for a contribution that is not currently installed is retained and clearly marked, never silently trusted or discarded. |
...
| CA-07 | **Trust, review status and permission are three separate axes** (`I-305`, `I-247`, `I-248`). A signature proves origin, a review status describes process, neither implies safety, and none of them grants permission. |
| CA-08 | **Community packages are never automatically trusted** (`I-325`). |
...
| SD-02 | **Public SDK contract ≠ internal LocalRpc contract** (`I-327`). |
| SD-03 | The SDK lives in the **Apache-2.0 interoperability boundary** (**D-021**), together with public wire schemas, public DTOs, public clients and contract-level validators. |
...
| SD-05 | **The SDK major version is separate from the extension protocol version** (`PK-05`). |
...
| DX-02 | **Developer Mode does not bypass permission, secret rules or workspace policy** (`I-271`). Its trust level is *lower*, not higher. |
...
| # | Requirement |
|---|---|
| PL-01 | **Mobile and web never load executable extensions.** No community executable plug-in is loaded into the mobile or web client. |
...
| PL-03 | **Cloud connector execution runs cloud-side** and **runs no third-party binaries** in the ArcForges cloud host by default; where a cloud-side extension ever exists it is isolated to the same standard as a cloud task (`RX-07`). |
...
**Extension search is not knowledge search** (`I-284` family). Finding a package is a catalog operation; finding user content is knowledge search. They are never the same box.
...
| **D-009** | Public SDK contracts split by boundary, ownership and licence |
| **D-021** | The public SDK surface sits inside the Apache-2.0 interoperability boundary |
| **V-02** | MCP `2026-07-28` stable status, statelessness, extension-framework vocabulary collision and the SDK version-pin gate |

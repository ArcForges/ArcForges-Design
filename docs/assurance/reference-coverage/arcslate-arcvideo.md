# Reference Coverage Matrix — ArcSlate / ArcVideo + ArcVideoFoundation + Olive

> Status: **Authoritative** — Phase 2 design-stage evidence · **Complete for the two accessible references; Olive unresolved (`§3.1`)**
> Governing authority: **D-012**, **D-013**, **D-002** (ArcSlate inherits product direction from a superseded product, not its model)
...
| Field | ArcVideo | ArcVideoFoundation | Olive |
|---|---|---|---|
| Repository | `github.com/ArcForges/ArcVideo` | `github.com/ArcForges/ArcVideoFoundation` | `github.com/olive-editor/olive` |
| Local path | `C:\MyFile\ArcForges\ArcVideo` | `C:\MyFile\ArcForges\ArcVideoFoundation` | **not present** |
| Commit | `caf5651` | `139eeca` | **not determinable** |
| Commit date | 2026-03-16 | 2026-03-30 | — |
| Reviewed on | 2026-09-05 | 2026-09-05 | **not reviewed** |
...
### 3.1 Olive — unresolved

| Field | Position |
|---|---|
| Status | **Unresolved determination** |
| Blocker | Olive is registered by **D-012** as an ArcSlate reference, but **no Olive repository exists in the authorized reference map location** (`C:\MyFile\ArcForges\`). A filesystem search of `C:\MyFile` to depth 4 found no candidate. |
| What was **not** done | No substitute repository was used, and no excluded historical plan was recovered, per the constraint on this repair. |
| What **is** available | Olive-derived evidence is reachable **transitively** through ArcVideo, which documents itself as an Olive fork (`§2`). The rows marked *(Olive-derived)* below rest on that fork, not on Olive directly. |
| Why the distinction matters | A fork at a point in time is not the upstream. Divergence after the fork point, and any Olive capability ArcVideo removed, are **not** covered by this matrix. |
| Affected scope | Olive capabilities absent from or altered in ArcVideo at `caf5651`; Olive's own licence file and its per-file provenance; Olive's tests and interchange fixtures. |
| Owner | Licensing and Provenance Owner, with the Product Owner |
| Blocks | Nothing in `WP-36`–`WP-39` that this matrix already covers through ArcVideo. It blocks only a claim of *complete Olive coverage*, which is not made. |
| Resolution | Either make Olive available at the authorized location and extend this matrix, or record an accepted exclusion for the Olive-direct scope with the Product Owner's approval. **This is one of the two items requiring the user's input** (see `§7`). |
...
| Every row has exactly one completeness state | **Pass** — 29 evidence established, 2 accepted exclusions (`AL-24` carries both), 0 unresolved **within the accessible references** |
...
| Olive coverage | **Unresolved** — `§3.1`. The matrix does not claim Olive coverage |

**Unresolved determinations: one — Olive (`§3.1`).**
...
## 7. Item requiring the user's decision

| Item | Question | Why it cannot be resolved here |
|---|---|---|
| Olive availability (`§3.1`) | **Olive is registered by D-012 as an ArcSlate reference but is not present at the authorized location.** Either make it available so this matrix can be extended, or accept an explicit exclusion for the Olive-direct scope. | Substituting a similar repository is prohibited by this repair's constraints; recovering an excluded historical plan is prohibited; and silently treating the fork as equivalent to the upstream would misstate the provenance chain. **D-012** registered Olive deliberately, so dropping it is a scope decision, not an implementation choice. |

Until it is resolved, ArcSlate planning proceeds on the two accessible references, and **no claim of complete Olive coverage is made anywhere**.
...
| MT-03 | The Olive unresolved determination is carried in [`../open-gates-register.md`](../open-gates-register.md) until decided. |

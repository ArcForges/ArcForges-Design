...
| AC-06 | ACP agent integration | `common/chat/acpToolCallOutput.ts`, `common/chat/slash/acpMapping.ts`, `common/types/agent/remoteAgentTypes.ts` | `08-extensions-and-developer-platform.md` §external agent; `WP-41.07` | Reference Only | Evidence of the external-agent adapter shape; ArcForges maps external agent work onto its own Task model with a capability lease | Apache-2.0; no reuse proposed | First-party test: external agent work produces ArcForges Tasks with leases | Architecture Owner | Evidence established |
...
| AC-11 | Teams / multi-agent | `docs/prds/teams`, `common/adapter/teamMapper.ts`, `renderer/pages/team` | **Accepted exclusion** — team capability is beyond the first launch in the accepted scope | Drop | The corpus places team capability past first launch; importing it now would create unrequested scope | Apache-2.0; no reuse | — | Product Owner | Accepted exclusion |

...
| <a id="rule-as-20"></a>AS-20 | gRPC API | `app/src/API/GRPC/**` | **Accepted source exclusion** — reference API implementation is not adopted | Drop | ArcForges uses its own authored proto/native gRPC under P2-009/010. The GPL reference implementation is not reused; this row does not prohibit gRPC or remove the accepted first-party protocol | GPL-3.0-only — no reuse | — | Architecture Owner | Accepted exclusion |

...
| # | Rule |
|---|---|
| RC-01 | **A disposition is never inferred from the repository-root licence** (**D-013**). Where a subtree carries a different licence, the subtree's licence governs. |
| RC-02 | **Reference capability ≠ ArcForges requirement.** An item maps to an existing requirement, or it is an accepted exclusion. It does not create a requirement by existing. |
| RC-03 | **`Reference Only` is the default disposition** where the licence prohibits reuse or where ArcForges' own design already governs the area. |
| RC-04 | **A `Copy`, `Rewrite`, `Improve` or `Replace` disposition additionally requires the full ten-field provenance record of D-013** before any material moves. No row in this matrix set carries such a disposition without that requirement stated. |
...
| Serial-Studio — GPL portion | **GPL-3.0-only** | **Not copyable, translatable or portable** under **D-013**. Reference Only |

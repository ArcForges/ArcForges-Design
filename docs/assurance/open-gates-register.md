...
| OG-05 | **Rule identifiers are document-scoped, and the collision is portfolio-wide.** `XX-nn` is defined **within one document** and means nothing outside it without a named home. This is not an accident to be tidied away: `BR-01` is a *Binding rule* in 53 different work packages by design, `RT-03` is defined in ten documents, `ID-01` and `EX-01` in eight each, and `GP-nn` names three unrelated families — repository projection (`§12` of the persistence architecture), GPU ownership (`§` GPU of the native-interop architecture) and gap reconciliation (`§4` of the realtime and bridge contract). Measured on this revision: **1,167 identifiers are defined in two or more normative documents**, and **1,597 citations of such an identifier occur in a document that defines none of them**. Two consequences bind. **(a) Every cross-document citation names its home** — “`PG-08` in the ArcSlate requirements”, “`GP-05` of the realtime contract”. **(b) A bare `XX-nn` means “in this document”**, and where a document defines no such identifier a bare citation is a defect, not a shorthand. `PG-nn` keeps its extra convention because this register is where gates live: unqualified, it is a gate. This is recorded as a known limitation with a measured size, and `PG-21` makes it mechanically checkable rather than a matter of care. |
...
| **PG-14b** | **Simulator acceptance against the real host**: same-seed hashes, fault positions, killed host with fenced takeover, duplicate commands, malformed AST and CSV, quota exhaustion, cross-workspace denial, realtime-disabled reconnect, partial cancellation and a 24-hour bounded-resource soak. **A preview or test fake is insufficient** (`SIM-20`) | Quality Owner with Architecture Owner | First ArcScope Cloud simulation release claim | Any ArcScope claim that Cloud simulation is a delivered capability. **It does not gate `WP-34`**, whose repeatable source is file/replay from `WP-33` (`SD-09` of the ArcScope requirements) | `WP-51.00`–`WP-51.05` | `OPEN` |
...
| **PG-17** | **Commit-order feed proof**: a change committed *after* a later one still receives a higher `publish_seq` and is delivered to a cursor that already advanced past it; two revisions of one aggregate published in the same batch arrive in revision order (`PB-06`); the claim scan is proven to return a contiguous prefix in `change_id` order **without** `FOR UPDATE` or `SKIP LOCKED` (`PB-07`); and a bootstrap taken while a business transaction is committing loses nothing, the duplicate being applied idempotently (`CU-02a`). The previous in-transaction sequence design could not provide the first of these, and no monotonic-identifier argument substitutes for the test (`PB-01`, `PB-02`) | Architecture Owner | First multi-writer sync deployment | Sync go-live; every client cursor guarantee | `WP-21.05`, `WP-25.02` | `OPEN` |
...
| **PG-21** | **Citation resolution proof**: a resolver run over the whole document set reports, for every `XX-nn` citation, the document that defines it, and **fails on any citation that resolves to zero definitions or to more than one without a named home** (`OG-05`). The 1,597 currently-ambiguous citations are worked to zero or explicitly waived one by one with a reason. **A spot check is not the evidence** — the defect this closes is precisely the one a reader does not notice, because a plausible wrong resolution reads correctly | Quality Owner with Architecture Owner | Before the first implementation package cites a design rule as an acceptance criterion | Any acceptance criterion that names a design rule; every completion gate citing another document | `WP-00.01` | `OPEN` |
...
| Gates created by Phase 2 | 21 | `PG-06` split into `PG-06` (design) and `PG-11` (implementation); `PG-12` added by the editing pass; **`PG-13`, `PG-14b`, `PG-15`, `PG-16` added by the P2-006 reconciliation**; `PG-17`–`PG-20` by the review-findings pass; **`PG-21` by the scope-propagation pass** |

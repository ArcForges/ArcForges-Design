...
| P-04 | Must-pass release scenarios pass for this product (`§27` there), including the **product's own** offline-start scenario: launch never requires an account (`ID-01`); for a Cloud-authoritative product, an **enrolled, hydrated** workspace opens editable with Cloud unreachable and **without an interactive re-authentication prompt**, pending edits visibly unsynchronised | Scenario results |
| P-05 | The product's **declared** exit path is met. **ArcScope and ArcSlate**: a portable package that re-imports completely and serialises deterministically (`WS-01`–`WS-06`; `WP-35.04`, `WP-39.02`). **ArcNotes and ArcChat**: a Cloud-generated download over acknowledged revisions with an attachment manifest and a stated fidelity/exclusion report — **re-import is not an obligation for these two** (`EP-04`, `EX-01`), so a round-trip result is not the evidence and must not be demanded | Round-trip result, or export completeness and fidelity report, per the product's declared path |

...
The first independent probe incorrectly demanded a response `+proto` suffix. The actual response is `application/grpc-web`, which [the gRPC-Web protocol](https://github.com/grpc/grpc/blob/master/doc/PROTOCOL-WEB.md) defines as default protobuf. The corrected probe accepts only the binary media type with an optional explicit protobuf suffix; all three real calls passed. Python urllib asset requests returned 403 with and without the local proxy; the browser-context request client successfully verified those same public bytes. Neither unsuccessful probe is reported as a pass or used to weaken a product requirement.

...
|---|---|---|---|
| F-001 | Conflict-resolution rule for the input corpus | `USER_CONFIRMED` | D-001 |
| F-002 | Product portfolio; ArcCanvas, ArcMusic, ArcImage | `USER_CONFIRMED` | D-002 |
| F-003 | ArcNotes complete scope | `USER_CONFIRMED` | D-006 |
| F-004 | Verification policy for time-sensitive claims | `USER_CONFIRMED` | D-003 |
| F-005 | Licensing boundary for ArcChat Mobile | `USER_CONFIRMED` | D-004 |
| F-006 | Web technology and rendering boundary | `USER_CONFIRMED` | D-007 |
| F-007 | Runtime and AOT matrix | `USER_CONFIRMED` | D-008 |
| F-008 | Contract granularity | `USER_CONFIRMED` | D-009 |
| F-009 | Cloud topology | `USER_CONFIRMED` | D-010 |
| F-010 | Target monorepo | `USER_CONFIRMED` | D-011 |
| F-011 | Reference-repository roles | `USER_CONFIRMED` | D-012 |
| F-012 | Reuse policy | `USER_CONFIRMED` | D-013 |
| F-013 | Reference-repository licences unknown | `DEFERRED_WITH_OWNER_AND_TRIGGER` | D-013, D-016 |
| F-014 | Payment provider baseline | `USER_CONFIRMED` | D-005 |
| F-015 | Web and service surface inventory | `USER_CONFIRMED` | D-014 |
| F-016 | Account portal URL | `USER_CONFIRMED` | D-015 |
| F-017 | Deferred-decision ownership | `USER_CONFIRMED` | D-016 |
| F-018 | Planning location and format | `USER_CONFIRMED` | D-017 |
| F-019 | Normative glossary | `USER_CONFIRMED` | D-018 |
| F-020 | Sequence status | `USER_CONFIRMED` | D-019 |
| F-021 | AI and payment economic model | `USER_CONFIRMED` | D-020 |
| F-022 | Apache boundary for validators and shared semantics | `USER_CONFIRMED` | D-021 |
| F-023 | Mobile provenance and dependency closure | `DEFERRED_WITH_OWNER_AND_TRIGGER` | D-004, D-016 |
| F-024 | Mobile-store commerce | `USER_CONFIRMED` | D-022 |
| F-025 | Mainland China payment route | `USER_CONFIRMED` | D-023 |
| F-026 | Refit AOT packaging and entry-point change | `DEFERRED_WITH_OWNER_AND_TRIGGER` | D-008, D-016; discovered by V-05c |

Totals: **26 registered** (F-001 to F-026, every identifier used, none skipped) = **23 resolved** · **3 validly deferred** (F-013, F-023, F-026) · **0 open** · **0 proposed**. One of the 26 was newly discovered during official verification (F-026), deferred as an implementation gate rather than a foundation conflict.

---

# Resolved issues

## F-001 — Conflict-resolution rule for the input corpus

**Type** `MISSING_DECISION` · **Status** `USER_CONFIRMED` · **Resolved by** D-001
...
## F-002 — Product portfolio and the disposition of ArcCanvas, ArcMusic, ArcImage

**Type** `CONTRADICTION` + `MISSING_DECISION` · **Status** `USER_CONFIRMED` · **Resolved by** D-002
...
## F-003 — ArcNotes complete scope

**Type** `CONTRADICTION` (apparent) + `MISSING_DECISION` (depth) · **Status** `USER_CONFIRMED` · **Resolved by** D-006
...
**Type** `EXTERNAL_FACT` · **Status** `USER_CONFIRMED` · **Resolved by** D-003, as amended by D-005
...
|---|---|---|
| Claude Sonnet 5 promotional price reverts to $3/$15 per MTok on 2026-08-31 | `I4 §Stage 8.15` | Date passed. Invalidated as a frozen figure by **D-020**. |
...
| Waffo Pancake `Change Subscription Product` returns 501 | `I4 §Stage 3.17`, `§4.63` | Obsolete per **D-005**. Historical evidence only. Not verified and never to be verified. |
| Waffo WeChat Pay documented inconsistently | `I4 §Stage 3.5` | Obsolete per **D-005**. Historical evidence only. The `BillingProviderCapabilities` principle it motivated survives independently. |
...
| Pinned dependency versions | `I3 §2` | Partially re-verified — see **V-05**. Refit carries a material change (**F-026**). |
| Android CoreCLR Native AOT not a stable production baseline; Mono AOT is production | `I3 §2.1`, `§29.6` | **VERIFIED — V-04.** |
| EF Core unsuitable as a strict-AOT production baseline | `I3 §2.1.4`, `§29.7` | **Moot for Cloud under D-008** — see V-05e. |
| ~30 pricing and quota figures | `I4 §Stages 0, 5, 8, 9, 10` | Deferred under D-003's first-consumption rule; frozen figures invalidated by **D-020**. |
...
This is the single active statement of D-003's applied scope. It replaces the earlier applied-scope paragraph in full.

**Verified now against current official primary sources** (all complete — see the verification record):

1. EU AI Act Article 50 applicability and current Commission transparency guidance — **V-01**.
2. MCP `2026-07-28` specification status and the official C# SDK — **V-02**.
3. .NET 10 ASP.NET Core Native AOT support and limitations — **V-03**.
4. .NET MAUI runtime and compilation status for Android and iOS — **V-04**.
5. AOT evidence for Avalonia, StreamJsonRpc, Refit, SignalR and the dependency set consumed by an AOT deliverable — **V-05**.
6. **Paddle** — Merchant-of-Record role, supported regions, currencies, hosted checkout, subscriptions, webhooks, refunds and payment methods — **V-06**.
7. **Payoneer** — the supported Paddle-to-Payoneer payout relationship — **V-07**.
8. **Paddle mainland-China support**, including Alipay and WeChat Pay — **V-08**.
9. Current Apple App Store and Google Play rules for a free consumption-only companion application — **V-09**.

**Deferred with a first-consumption trigger, not verified now:** all provider pricing, quotas, fee schedules, payout terms, storage and AI rates, exchange rates, tax rates, Azure regional availability tables, and store fee details. Consistent with `I3 §32` and `I4 §Stage 8.77`, which already forbid acting on an unreviewed price change. **Owner:** Commercial Operations Owner (D-016). **Trigger:** first authoritative pricing specification, and again before launch.
...
**D-005 replaced the Waffo Pancake item in D-003's original applied verification worklist with Paddle and Payoneer.** The verbatim D-003 decision block is unchanged, and every other part of D-003 — its scope split, its deferral rule, and its first-consumption trigger — remains in force exactly as recorded. The scope statement above is the current effective form.

## F-005 — Licensing boundary for ArcChat Mobile

**Type** `CONTRADICTION` — not raised anywhere in the corpus · **Status** `USER_CONFIRMED` · **Resolved by** D-004
...
**Register hygiene note** During drafting, a derived issue about shared-assembly licensing was prepared under the identifier F-022 but **was never written to disk**. It is superseded in substance by D-004. It is recorded here only as a never-created draft; F-022 below is a different, separately decided question.

## F-006 — Web technology and rendering boundary

**Type** `CONTRADICTION` · **Status** `USER_CONFIRMED` · **Resolved by** D-007
...
## F-007 — Runtime and AOT matrix

**Type** `CONTRADICTION` / `NEEDS_OFFICIAL_VERIFICATION` · **Status** `USER_CONFIRMED` · **Resolved by** D-008
...
**Verification** **V-03** confirms the conflict is real and structural: under .NET 10, "Other Authentication", MVC, Blazor Server, OData, Session and Spa are **Not supported** under Native AOT; Minimal APIs and SignalR have only **Partial support**. **V-04** confirms Android CoreCLR is experimental and not intended for production in .NET 10.
...
**Corpus correction recorded** `I3 §2.1.3`'s statement that SignalR is unsupported under AOT reflects the .NET 8 status. Under .NET 10 SignalR has Partial support (V-03). The decision is unaffected; the stale statement must not be carried forward unamended.

## F-008 — Contract granularity

**Type** `CONTRADICTION` (mild) · **Status** `USER_CONFIRMED` · **Resolved by** D-009

**Where** `I3 §5.1` and `§5.3` define four shared Contracts projects with all LocalRpc interfaces in one assembly. `I3 §6.5` itself warns local contracts must stay small rather than becoming "one ever-expanding giant assembly". `I4 §Stage 21.122–123` requires per-product semantic ownership and versioning, so an ArcNotes contract change must not force an ArcSlate re-release. D-004 adds the constraint that mobile-facing contracts must sit in clearly bounded Apache-2.0 packages.
...
## F-010 — Target monorepo

**Type** `MISSING_DECISION` · **Status** `USER_CONFIRMED` · **Resolved by** D-011
...
## F-011 — Reference-repository roles

**Type** `UNSUPPORTED_ASSUMPTION` · **Status** `USER_CONFIRMED` · **Resolved by** D-012
...
**Type** `USER_INTENT` / `MISSING_DECISION` · **Status** `USER_CONFIRMED` · **Resolved by** D-013

**Where** `I2 §II` states as user-confirmed that non-AGPL content from AFFiNE, Serial Studio, ArcVideo, ArcVideoFoundation and Olive may be copied — code, logic, tests and assets — then progressively replaced. Its interaction with the `I4 §Stage 11.5–7` licence traffic lights (GPL-2.0-only and unknown-origin code are `RED`) was unstated. D-004 adds a hard constraint on the mobile side.

**Outcome** Copy First is retained only as a **licence-gated and provenance-gated** strategy; unconditional copying is rejected. A nine-field provenance record is required before any reuse.

## F-014 — Payment provider baseline

**Type** `CANDIDATE_ARCHITECTURE` + `NEEDS_OFFICIAL_VERIFICATION` · **Status** `USER_CONFIRMED` · **Resolved by** D-005
...
**Outcome** Waffo Pancake is removed. Paddle is the sole customer-facing Merchant of Record for web and cloud commerce (**VERIFIED — V-06**); Payoneer is the payout and settlement destination (**VERIFIED — V-07**).
...
V-08 makes `BillingProviderCapabilities` load-bearing rather than precautionary: Alipay and WeChat Pay differ from each other and from cards on subscription support, currency, platform, chargeback availability and transaction caps.
...
## F-015 — Web and service surface inventory

**Type** `MISSING_DECISION` (minor) · **Status** `USER_CONFIRMED` · **Resolved by** D-014
...
## F-016 — Account portal URL

**Type** `CONTRADICTION` · **Status** `USER_CONFIRMED` · **Resolved by** D-015

**Where** `I4 §Stage 1.44` prefers `arcforges.com/account`; `§Stage 2.1` formally chooses `account.arcforges.com` with `arcforges.com/account` as a permanent redirect. Reopened under D-001 because internal corpus resolution is not self-authorising.
...
## F-017 — Deferred-decision ownership

**Type** `MISSING_DECISION` · **Status** `USER_CONFIRMED` · **Resolved by** D-016

**Where** The corpus names no owners. The Phase 1 completion gate requires every deferred decision to carry a valid owner and trigger. F-013 and F-023 previously named the user as owner by default, which D-016 explicitly rejects as an operational model.
...
## F-018 — Planning location and format

**Type** `OBSOLETE` / `MISSING_DECISION` · **Status** `USER_CONFIRMED` · **Resolved by** D-017
...
## F-019 — Normative glossary

**Type** `MISSING_DECISION` · **Status** `USER_CONFIRMED` · **Resolved by** D-018
...
**Verification consequence** **V-02** adds a term-collision risk from outside the corpus: MCP's extension framework defines **Tasks** and **Skills**, which overlap the ArcForges `Task / Run / Step / Attempt` vocabulary from `I4 §Stage 19`. The glossary must disambiguate MCP-`Task` from ArcForges-`Task` explicitly.

## F-020 — Sequence status

**Type** `SEQUENCING_PROPOSAL` · **Status** `USER_CONFIRMED` · **Resolved by** D-019
...
## F-021 — AI and payment economic model

**Type** `EXTERNAL_FACT` · **Status** `USER_CONFIRMED` · **Resolved by** D-020
...
## F-022 — Apache-2.0 boundary for validators, application semantics and base ViewModel patterns

**Type** `MISSING_DECISION`, arising from D-004 · **Status** `USER_CONFIRMED` · **Resolved by** D-021
...
## F-024 — Mobile-store commerce

**Type** `MISSING_DECISION`, arising from D-005 · **Status** `USER_CONFIRMED` · **Resolved by** D-022
...
## F-025 — Mainland China payment route

**Type** `MISSING_DECISION`, newly exposed by D-005 · **Status** `USER_CONFIRMED` · **Resolved by** D-023
...
**Verification** **V-08** establishes the real capability set. Alipay: one-time and subscriptions supported, CNY only, 1,600 CNY cap on renewals and charges, separate Paddle approval required, no chargebacks, no saved payment methods. WeChat Pay: one-time only — **no subscriptions** — CNY or USD, **desktop only**, no configuration required, no chargebacks, no saved payment methods. Neither requires a Chinese entity or a provider merchant account.

**Two corrections to the corpus, both absorbed by D-023 without amendment**

1. **CNY pricing inverts from forbidden to required.** `§Stage 3.6` forbade a fixed CNY price. Paddle's Alipay route requires CNY-priced products and conditions approval on it. D-023's "CNY product and tax configuration" gate already carries this.
2. **The cap changed shape.** The corpus recorded ~$140 / ¥1,000 single-transaction for the removed provider; Paddle's Alipay cap is 1,600 CNY on subscription renewals and charges. D-023's instruction not to hard-code caps already carries this.

**Outcome** Mainland China is a conditional launch market. No second payment provider for V1. Paddle-hosted checkout with Alipay, WeChat Pay and available cards; Payoneer as payout only. Cloud Pass remains the non-recurring fixed-term product — a requirement made concrete by V-08, since WeChat Pay users cannot subscribe at all and Alipay users hit a renewal ceiling. Eight pre-enablement gates; on failure, disable regional sales by explicit policy without blocking global launch and without silently substituting another provider.

---

# Deferred issues

Each carries a responsible role, a concrete trigger, the earliest consumer, required evidence, and the consequence if the gate fails — per **D-016**.

## F-013 — Reference-repository licences unknown

**Status** `DEFERRED_WITH_OWNER_AND_TRIGGER` · **Governed by** D-013, D-016
...
| **Failure consequence** | The material is classified Reference Only or Drop. It must not be copied, translated or ported. Per D-013, GPL-only, licence-unclear and unknown-origin material may be used only as controlled behavioral evidence until an explicit compatibility decision says otherwise. |

## F-023 — Mobile provenance and dependency-closure verification

**Status** `DEFERRED_WITH_OWNER_AND_TRIGGER` · **Governed by** D-004, D-016

**Where** D-004: "The current decision assumes that ArcChat Mobile and its required interoperability code are ArcForges-owned and contain no GPL-family code. Verify provenance and the complete dependency closure before distribution."
...
| **Required evidence** | Ownership or Apache-2.0-compatible licence for the application, mobile-only libraries, packaging and platform integrations, and every Apache-2.0 interoperability package consumed; complete **direct and transitive** dependency closure compatible with Apache-2.0 application distribution and applicable store terms; passing automated architecture and dependency checks per D-004 obligation 7 |
| **Failure consequence** | Per D-004, register that specific issue and return it for decision. **Never** silently add an exception, change the licence, or remove the mobile distribution target. |

## F-026 — Refit AOT packaging and entry-point change

**Status** `DEFERRED_WITH_OWNER_AND_TRIGGER` · **Governed by** D-008, D-016 · **Discovered by** V-05c
...
## D-001 — Conflict-resolution rule · resolves **F-001** · `USER_CONFIRMED`
...
## D-002 — Product baseline · resolves **F-002** · `USER_CONFIRMED`
...
## D-003 — Verification scope · resolves **F-004** · `USER_CONFIRMED`

> Foundation-critical only (Recommended)

The current effective applied scope of this decision is recorded under **F-004**, together with the D-005 supersession history. The verbatim decision block above is unchanged.

---

## D-004 — ArcChat Mobile licensing boundary · resolves **F-005** · `USER_CONFIRMED`
...
### D-004 obligations carried into the specification phase
...
| 2 | Separate mobile-facing contracts out of any broader AGPL-only project into clearly bounded Apache-2.0 projects or packages | Solution and project layout; governed by D-009 |
...
| 8 | Verify provenance and the complete direct and transitive dependency closure before distribution | Tracked as **F-023** |

---

## D-005 — Payment provider baseline · resolves **F-014** · `USER_CONFIRMED`
...
>
> Apply this as a global disposition. Mark F-014 USER_CONFIRMED and resolved by D-005.
>
> Do not rewrite or alter the verbatim D-003 decision block. Add a clear supersession note stating that D-005 replaces only the Waffo Pancake part of D-003’s applied verification worklist. The current verification target is Paddle and Payoneer, using current official primary sources. Verify their current availability, onboarding eligibility, supported regions and currencies, payout relationship, API/webhook capabilities, and any implementation-critical restrictions. Pricing and fees remain time-sensitive and must follow D-003’s first-consumption rule.
>
...
## D-006 — ArcNotes complete scope · resolves **F-003** · `USER_CONFIRMED`
...
>
> Mark F-003 USER_CONFIRMED and resolved by D-006.
...
>
> Mark F-006 USER_CONFIRMED and resolved by D-007.

---

## D-008 — Runtime and AOT matrix · resolves **F-007** · `USER_CONFIRMED`
...
>
> Mark F-007 USER_CONFIRMED and resolved by D-008.

**Instruction honoured.** No Azure SDK Native AOT verification was attempted (V-05e).

---

## D-009 — Contract granularity · resolves **F-008** · `USER_CONFIRMED`
...
>
> Mark F-008 USER_CONFIRMED and resolved by D-009.

---

## D-010 — Cloud topology · resolves **F-009** · `USER_CONFIRMED`
...
>
> Mark F-009 USER_CONFIRMED and resolved by D-010.

---

## D-011 — Target monorepo · resolves **F-010** · `USER_CONFIRMED`
...
## D-012 — Reference-repository roles · resolves **F-011** · `USER_CONFIRMED`
...
>
> Keep F-013 DEFERRED_WITH_OWNER_AND_TRIGGER. Its trigger is the first step of the per-product Reference Coverage Matrix and licence audit, before substantive reference source is used for planning or implementation.
>
> Mark F-012 USER_CONFIRMED and resolved by D-013.

---

## D-014 — Web and service surface inventory · resolves **F-015** · `USER_CONFIRMED`
...
>
> Mark F-015 USER_CONFIRMED and resolved by D-014.

---

## D-015 — Account portal URL · resolves **F-016** · `USER_CONFIRMED`
...
>
> Mark F-016 USER_CONFIRMED and resolved by D-015.

---

## D-016 — Deferred-decision ownership · resolves **F-017** · `USER_CONFIRMED`
...
>
> - F-013 → Licensing and Provenance Owner; trigger: before reference material is reused.
> - F-023 → Release Engineering plus Licensing and Provenance Owner; trigger: before the first mobile distributable.
...
>
> Mark F-017 USER_CONFIRMED and resolved by D-016.

**Applied.** F-013, F-023 and F-026 above carry role, approval authority, trigger, earliest consumer, required evidence and failure consequence. The pricing/fee deferral is recorded under F-004. The dependency AOT proofs are recorded in the verification record's deferred-gate table.

---

## D-017 — Planning location and format · resolves **F-018** · `USER_CONFIRMED`
...
>
> Mark F-018 USER_CONFIRMED and resolved by D-017.

---

## D-018 — Normative glossary · resolves **F-019** · `USER_CONFIRMED`
...
>
> Mark F-019 USER_CONFIRMED and resolved by D-018.

**Gate recorded.** Not generated in this turn. **Owner:** Architecture Owner. **Trigger:** before any detailed product specification is finalized. **Additional input from V-02:** MCP's extension framework defines Tasks and Skills, which collide with the ArcForges execution vocabulary and must be disambiguated explicitly.

---

## D-019 — Sequence status · resolves **F-020** · `USER_CONFIRMED`
...
>
> Mark F-020 USER_CONFIRMED and resolved by D-019.

---

## D-020 — AI and payment economic model · resolves **F-021** · `USER_CONFIRMED`
...
>
> Mark F-021 USER_CONFIRMED and resolved by D-020.
...
>
> Mark F-022 USER_CONFIRMED and resolved by D-021.

---

## D-022 — Mobile-store commerce · resolves **F-024** · `USER_CONFIRMED`
...
>
> Mark F-024 USER_CONFIRMED and resolved by D-022.

---

## D-023 — Mainland China payment route · resolves **F-025** · `USER_CONFIRMED`
...
>
> Mark F-025 USER_CONFIRMED and resolved by D-023.

**Verified against V-08.** The intended route matches Paddle's documented capability set item for item. V-08 adds three operational facts that sharpen but do not change the decision: Alipay requires a separate approval application conditioned on CNY pricing; the Alipay cap is 1,600 CNY on renewals and charges; and **neither** China method supports chargebacks, so the dispute and refund model must not assume the card pathway.

---

# Dispositions applied

Global rules now in force. Applied to all occurrences they cover without further asking, per D-001.

| # | Disposition | Authority | Scope |
|---|---|---|---|
| 1 | No automatic precedence among raw input documents; they are evidence only. Conflicts are registered, explained, decided, recorded, then applied globally. | D-001 | All |
| 2 | Only material conflicts not already covered by an existing user decision are brought back. | D-001 | All |
| 3 | The product baseline is exactly ArcChat, ArcNotes, ArcScope and ArcSlate. | D-002 | All |
...
| 11 | No App Store exception, dual licensing, proprietary grant or CLA for ArcChat Mobile. DCO continues with inbound-equals-outbound licensing per scope. | D-004 | T20 |
| 12 | ArcChat Mobile must not contain, link to, copy from, port from or reference any GPL-family or AGPL-only implementation, directly or transitively. | D-004 | T17, T20 |
| 13 | Protocol communication across explicit process or network boundaries does not change the mobile client's licence; desktop and server implementations remain separate works. | D-004 | T07, T17, T20 |
| 14 | On discovery of a conflicting contribution or dependency in the mobile boundary: register that specific issue and return it for decision. Never silently add an exception, change the licence, or remove the mobile distribution target. | D-004 | T20 |
| 15 | **Waffo Pancake is obsolete and `SUPERSEDED` as a provider choice.** It must not be verified, recommended, integrated, or included in any new authoritative specification, roadmap, dependency, runtime component or implementation step. Every reference to it in the raw corpus is historical evidence only. | D-005 | T11, T12 |
| 16 | **Paddle is the sole customer-facing Merchant of Record for ArcForges web and cloud commerce**, owning checkout, subscriptions, recurring billing, applicable sales-tax and VAT handling, compliant invoices, refunds, chargebacks and payment webhooks. | D-005 | T11 |
...
| 24 | Desktop remains a Native AOT deliverable with trim/AOT-safe dependency rules. **Only projects actually consumed by an AOT deliverable must satisfy AOT release gates.** Shared public contracts and client libraries stay trim-safe and source-generation friendly. | D-008 | T07, T19 |
| 25 | ArcChat Mobile Android uses the supported .NET 10 Mono AOT release path. Experimental Android CoreCLR and experimental Android NativeAOT are not production baselines. iOS architecture stays present and complete with its build deferred; its release runtime is re-verified against the then-current supported MAUI/iOS baseline. | D-008 | T17 |
| 26 | Contracts are split by communication boundary, product/domain ownership, release cadence and licence boundary. A single ever-growing contracts assembly is rejected, and an ArcNotes contract change must not force an unrelated ArcSlate release. | D-009 | T07, T20 |
| 27 | C# DTOs and endpoint metadata are the source of truth; OpenAPI and JSON Schema compatibility artifacts are generated from them. No parallel handwritten schemas. No business implementation in a contracts package. | D-009 | T07 |
...
| 35 | The twelve-entry web and service surface inventory in D-014 is the consolidated list. A hostname is not an application: Account and Chat may be separate deployments of one `ArcForges.Web.App` codebase, and static surfaces stay static artifacts. | D-014 | T16 |
| 36 | `account.arcforges.com` is the canonical account portal; `arcforges.com/account` is a permanent redirect and never a second account application. Explicit origin, cookie, OAuth redirect, CSP, CSRF and CORS boundaries; no broad parent-domain authentication cookies. | D-015 | T16, T10 |
| 37 | **"The user" is not an operational owner.** Every deferred item names a responsible role, the Product Owner as final approval authority where a product decision is required, a concrete trigger, the earliest consumer, required evidence, and the consequence if the gate fails. Roles are durable: Architecture Owner, Product Owner, Licensing and Provenance Owner, Security/Privacy Owner, Commercial Operations Owner, Release Engineering Owner. | D-016 | All |
| 38 | Authoritative output locations are fixed: `docs/requirements/`, `docs/architecture/`, `docs/decisions/`, `docs/assurance/`, `docs/planning/work-packages/`. The old `ArchitectureDesign` locations are obsolete, must never be used as an output or authority, and must not be accessed or depended on. `AionUiReWrite-Kotlin` is not an input corpus or authority. | D-017 | T23 |
...
| 40 | Every corpus sequence is planning evidence only. The implementation plan is one serial numbered sequence `00 → 01 → … → NN` with no predetermined maximum, derived after requirements, architecture, licence matrices and current-code reconciliation. Work interleaves by real dependency gates. One main context advances it serially; implementation ownership is never split across autonomous agent teams. | D-019 | T23 |
| 41 | The provider-independent accounting model is preserved: fixed-precision credit accounting, versioned retail tariffs, per-run tariff snapshots, three separate ledgers, reserve-then-settle, immutable historical financial records, explicit hard stop at exhausted balance. | D-020 | T11, T12 |
| 42 | Every frozen economic figure derived from expired model pricing or the removed provider's fees is invalidated and removed from the authoritative baseline, and is **not** replaced with new frozen numbers in Phase 1. Prices, allowances, pack sizes, margins and regional amounts are versioned commercial policy requiring approval at first specification consumption and again before launch. | D-020 | T11, T12 |
| 43 | The Apache-2.0 boundary is drawn around **interoperability**: public wire schemas, public/mobile DTOs, public clients, validation rules expressing wire-format constraints, public protocol state semantics, and the future public SDK. Product-domain behaviour, server orchestration, desktop use cases, policy decisions, persistence behaviour, entitlement authority and UI scaffolding stay outside it. | D-021 | T17, T20, T07 |
| 44 | **Base ViewModel patterns are not shared between Avalonia desktop and MAUI mobile.** Each UI stack owns its implementation. | D-021 | T17, T19 |
| 45 | **ArcChat Mobile is a free companion / consumption-only application.** It must not sell anything in-app, embed Paddle checkout, integrate StoreKit or Play Billing for initial release, display an external purchase button, link or call to action, or unlock functionality via a locally entered licence key or purchase token. All initial commerce occurs on the web through Paddle. | D-022 | T11, T17 |
| 46 | The same conservative mobile-commerce behaviour applies across all storefronts, even where a regional programme permits external links. The entitlement architecture stays capable of accepting a future store-originated grant, but that source is not implemented until a new explicit decision authorizes mobile purchasing. | D-022 | T11, T17 |
| 47 | Mainland China is a conditional launch market served through Paddle-hosted web checkout with the methods Paddle currently makes available. No second payment provider for V1, and no silent substitution if gates fail — regional sales are disabled by explicit policy without blocking global launch. | D-023 | T11 |
| 48 | **Provider caps, currencies, approval rules and platform limitations must never be hard-coded into domain contracts.** They are represented through `BillingProviderCapabilities` and verified commercial configuration. Cloud Pass remains the non-recurring fixed-term product. | D-023 | T11 |
| 49 | Absence of an official AOT guarantee is never treated as proof of AOT compatibility. Where documentation cannot prove a dependency's complete behaviour under an AOT deliverable, a real publish-and-test proof is registered as a deferred gate with an owner and trigger. | D-003, D-008 | T07, T19 |
...
|---|---|---|---|
| 1 | **Refit's reflection request builder moved to a separate package; `RestService.ForGenerated<T>` is the AOT-safe entry point; `RF006` signals a non-generable shape.** The corpus records none of this. | V-05c | **New issue F-026**, deferred as an implementation gate under D-008 and D-016. Changes no decision. |
| 2 | MCP `2026-07-28` is stable, not a release candidate. `I4 §Stage 6.41`'s prohibition was conditioned on RC status and lapses on its own terms. | V-02 | Recorded under F-004. No decision required — the corpus's own condition resolved it. |
| 3 | MCP's extension framework defines **Tasks** and **Skills**, colliding with the ArcForges `Task / Run / Step / Attempt` vocabulary. | V-02 | Added as required input to the D-018 glossary gate. |
| 4 | SignalR moved from Not supported (.NET 8) to **Partial support** (.NET 10) under Native AOT, making `I3 §2.1.3` stale. | V-03 | Recorded as a corpus correction under F-007. Decision unaffected — Cloud is JIT under D-008. |
| 5 | **CNY pricing inverts from forbidden to required.** `I4 §Stage 3.6` forbade a fixed CNY price; Paddle's Alipay route requires CNY-priced products and conditions approval on it. | V-08 | Absorbed by D-023's existing "CNY product and tax configuration" gate. |
| 6 | Neither Alipay nor WeChat Pay supports chargebacks, and WeChat Pay supports neither subscriptions nor mobile. | V-08 | Absorbed by D-023's instruction not to hard-code provider limitations and by its retention of Cloud Pass. Sharpens the refund and dispute model. |
| 7 | Apple 3.1.3(f) category fit is decided by App Review, not by reading the guideline; ArcChat Mobile fits the category's shape but is not literally enumerated. | V-09 | `PARTIALLY_VERIFIED`. Covered by the existing store-submission gate. Does not affect D-022, which already chooses the most conservative posture available. |
...
| 2 | Every deferred item has a responsible role and trigger | **Pass** — F-013, F-023, F-026 each carry role, approval authority, trigger, earliest consumer, evidence and failure consequence per D-016 |
| 3 | Official verification artifact complete | **Pass** — `docs/assurance/phase-1-official-verification.md`, V-01 to V-09 |
...
**Decisions recorded:** D-001 through D-023, verbatim.
